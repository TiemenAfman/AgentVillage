...
import { DATA, readJson, writeJsonAtomic, loadConfig, islandCap } from './paths.mjs';
...
function where(opts = {}) {
  ...
  return {
    ...
    // The most the island may be: what the reach below is measured against. The grid the
    // ground is built on is `canvas` - a berth's own, else the layout's (see ground()),
    // because an island that has grown stands on a bigger grid than it was founded on.
    gridSize: opts.gridSize ?? islandCap(config),
    canvas: opts.gridSize ?? null,
...
const MAX_GROUNDS = 8;
function ground(w) {
  const layout = w.polders === null || w.paved === null || w.grow === null || w.canvas === null ? (readJson(w.layoutFile, null) || {}) : {};
  const size = w.canvas ?? layout.size ?? w.gridSize;
  ...
  const key = `${w.seed}:${size}:${JSON.stringify(polders)}:${fairway ? fairway.cells.length : 0}:${w.paved ? `p${w.paved.length}` : (layout.paths ? layout.paths.length : 0)}:${grow ? JSON.stringify(grow) : ''}`;
  const hit = cached.get(key);
  if (hit) return hit;

  const t = makeTerrain(w.seed, { size, polders, fairway, grow });
  const paved = new Set();
  if (w.paved) {
...
// The check a bed gets, for anything else that has to stand on honest ground: the marks the
// story animals leave (lib/animal-places.mjs, Plans/dierenverhalen.md). One copy of "not in
// the sea, the river, on the sand, on the flagstones or up a cliff", so a nest can never be
// put where a turnip would have been refused. Pass `seed`, `gridSize`, `polders`, `fairway`,
// `grow` and `paved` and nothing is read from disk but the config.
export function groundCheck(opts = {}) {
  const here = ground(where(opts));
  return { terrain: here.t, refuse: (x, z) => whyNot(here, x, z) };
}
