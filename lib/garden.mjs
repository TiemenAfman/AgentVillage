...
// island in play: this island and up to four berths, each with its own seed, size and
// reclaimed land. One slot meant alternating between the host's garden and a guest's
// rebuilt the terrain on every single call. Eight is the host plus twice the berths there
// can be (lib/guests.mjs: MAX_GUEST_ISLANDS is 4), and the oldest is dropped so a run of
// visits cannot grow this without bound.
const cached = new Map();
const MAX_GROUNDS = 8;
function ground(w) {
  const layout = w.polders === null || w.basins === null || w.paved === null ? (readJson(w.layoutFile, null) || {}) : {};
  const polders = w.polders ?? layout.polders ?? [];
  // The quay is water now, and a bed floated out in the harbour is not a bed. Read the
  // same way the polders are, and for the same reason: a berth has no layout.json.
  const basins = w.basins ?? layout.basins ?? [];
  ...
  const key = `${w.seed}:${w.gridSize}:${JSON.stringify(polders)}:${JSON.stringify(basins)}:${w.paved ? `p${w.paved.length}` : (layout.paths ? layout.paths.length : 0)}`;
  const hit = cached.get(key);
  if (hit) return hit;

  const t = makeTerrain(w.seed, { size: w.gridSize, polders, basins });
  const paved = new Set();
  if (w.paved) {
...
// when the purse or the basket will not stretch. That is load-bearing far away from here:
// a visit travels home as a log of buys and sells rather than as a purse balance, and
// this is what makes "no longer affordable at home" a refusal that can be reported
// instead of a balance forced negative. See lib/visits.mjs.
