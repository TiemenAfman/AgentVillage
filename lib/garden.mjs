...
function ground(w) {
  ...
  const key = `${w.seed}:${w.gridSize}:${JSON.stringify(polders)}:${w.paved ? `p${w.paved.length}` : (layout.paths ? layout.paths.length : 0)}`;
  const hit = cached.get(key);
  if (hit) return hit;

  const t = makeTerrain(w.seed, { size: w.gridSize, polders });
  const paved = new Set();
  if (w.paved) {
