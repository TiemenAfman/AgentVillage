...
const MAX_GROUNDS = 8;
function ground(w) {
  const layout = w.polders === null || w.paved === null ? (readJson(w.layoutFile, null) || {}) : {};
  ...
  const key = `${w.seed}:${w.gridSize}:${JSON.stringify(polders)}:${fairway ? fairway.cells.length : 0}:${w.paved ? `p${w.paved.length}` : (layout.paths ? layout.paths.length : 0)}`;
  const hit = cached.get(key);
  if (hit) return hit;

  const t = makeTerrain(w.seed, { size: w.gridSize, polders, fairway });
  const paved = new Set();
  if (w.paved) {
