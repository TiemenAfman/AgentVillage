// What happens to the island's story animals (Plans/dierenverhalen.md, docs/next/animal-stories.md).
//
...
import {
  STORY_SPECIES, MAX_ANIMALS, ARRIVAL_ORDER, TRAITS, OPPOSITES, TRAIT_POOLS, NAMES, TRACE_KINDS,
...
export function rulesFor(pace = 1) {
  ...
  return {
    ...
    abandonAfter: (3 * HOUR_MS) / p,       // an errand the sea never finished
...
// The mystery (docs/next/animal-stories.md, "Permanent traces, combinations and mysteries").
...
function applyArrival(s, d, at) {
  ...
  put(s.animals, d.id, {
    ...
    favourite: null, lastWindow: -1, lastRestWindow: -Infinity, majorDay: -1, encounters: 0, rel: {},
  });
...
const rngFor = (s, purpose) => makeRng(hash32(`animal:${s.seq + 1}:${purpose}`));
...
function decideObserve(s, c, at, rules) {
  ...
  if (animals.length) {
    ...
    if (s.mystery && s.mystery.stage === 2 && c.landmark && spot(c.landmark.at) &&
      ...
      const free = animals.filter((a) => !busy.has(a.id));
...
function spotFor(t, r, animal) {
  ...
  const sp = r.spots || {};
  return spot(sp[t.where]) ? sp[t.where] : null;
}
...
// ---- reading ---------------------------------------------------------------------------
// The words an event says in the diary, and who and where it is about.
export function entryOf(e) {
  const d = e.data || {};
  const base = { seq: e.seq, at: e.at, kind: e.kind };
  if (e.kind === 'arrival') return { ...base, animal: d.id, line: d.line, where: [d.home.x, d.home.z], big: true };
  if (e.kind === 'encounter') {
    return { ...base, action: d.action, line: d.line, major: !!d.major, rel: d.rel || null,
      traces: (d.traces || []).map((t) => t.id), discovery: d.discovery ? d.discovery.id : null,
      mystery: d.mystery ? (d.mystery.stage === 1 ? 'print' : d.mystery.find ? 'find' : d.mystery.stage === 3 ? 'reveal' : null) : null,
      big: !!(d.major || (d.traces && d.traces.length) || d.discovery || (d.mystery && !d.mystery.dry && d.mystery.dry !== 0)) };
  }
  if (e.kind === 'vignette') return { ...base, animal: d.animal, line: d.line, big: false };
  return null;
}
