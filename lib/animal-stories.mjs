...
export function rulesFor(pace = 1) {
  ...
  return {
    ...
    // Island encounters before the first clue: about two working days at three an hour. At
    // eight, the whole mystery - print, three finds, the reveal - was over inside one day.
    mysteryAfter: 40,
    // A find's chance on a notable encounter: with a settler who has not given one yet, with
    // one who has, and the run of empty-handed visits after which one always turns up.
    findNew: 0.2,
    findAgain: 0.08,
    findPity: 8,
    maxFeeders: 3,
...
function mysteryCheck(s, animal, a, data, rng, rules, landmark) {
  ...
  const p = m.dry >= rules.findPity ? 1 : found.has(a.resident) ? rules.findAgain : rules.findNew;
  if (rng.next() < p) {
