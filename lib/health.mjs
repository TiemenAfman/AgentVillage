...
// guard's reach, and later a lava cell or another player's blow, are all decided here - so
...
//
// `hurt` is the one door for everything that harms a player. Guards call it now
// (lib/hostility.mjs); lava and players will call it later, with nothing else to learn.
//
...
// stream is never a place to heal.
export const REGEN_AFTER_MS = 3000;
// How fast, once it does: a proposal (the number is still open in the plan). Four seconds
// from nothing to whole, after the three of quiet.
...
// `oneHit` is what the island does today: every hit that lands is fatal and sends you back
// at once, whatever `amount` says. It is an option rather than a constant only so the tests
// can exercise the arithmetic the health bar will need; turning the bar on for real is
// flipping its default, and no caller of `hurt` changes.
export function createHealth({ fleet, roster, now = () => Date.now(), oneHit = true }) {
  ...
  // message every frame. Only said when there is something to say: a body the page has
  // never been told is hurt is whole as far as it knows, which is why a one-hit capture
  // sends nothing beyond the `evicted` it always did.
  function tell(p, b, t) {
  ...
  // `amount` in health points, `cause` = { kind, island }: what did it ('guard', later
  // 'lava' and 'player'), and the name of the place, which the page shows as where you were
