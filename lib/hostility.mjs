...
// Everybody on a boat: the pilot, and the crew beside them once a boat carries one
// (lib/boats.mjs, Plans/lopen-op-de-boot.md). Aboard is not afoot - no guard swims out to a
// deck, no lava reaches one, nobody swings from one. Whether a passenger should be fair game
// for the volcano's guards is an open question in that plan; this is the answer until it is
// decided. Read off the snapshot, which carries `crew` whenever a boat has one.
export const pilotsOf = (roster) => {
  const out = new Set();
  for (const b of roster.boats.snapshot()) {
    if (b.pilot) out.add(b.pilot);
    for (const c of b.crew || []) out.add(c);
  }
  return out;
};
