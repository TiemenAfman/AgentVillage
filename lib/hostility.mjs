...
export function createHostility({ fleet, crowds, roster, now = () => Date.now() }) {
  ...
  function tick() {
    ...
    const afloat = roster.boats.snapshot();
    const pilots = new Set(afloat.map(b => b.pilot).filter(Boolean));
    // Somewhere to be sent back to. An islander's is their own town square; a wanderer's -
    // the app on a phone, with no island at all - is their skiff (lib/boats.mjs), and a
    // wanderer who has not launched one has nowhere to go and so is left alone, as before.
    const skiffs = new Map(afloat.map(b => [b.id, b]));
    const sendable = p => !!fleet.get(p.island) || skiffs.has(skiffOf(p.id));
    ...
    for (const island of fleet.all()) {
      ...
      const visitors = players.filter(p => {
        if (!p.walking || p.island === island.id || !sendable(p) || p.room || pilots.has(p.id) || (p.f & 2) || immunity.has(p.id)) return false;
      ...
      for (const f of candidates) {
        ...
        if (distance < .65 && Math.abs(target.y - f.y) < 1.2) {
          ...
          if (home) {
            ...
            // Back in their own boat, wherever it floats. The boat's id rides along so the
            // page climbs straight in rather than being set down on the water beside it.
            const skiff = skiffs.get(skiffOf(target.id));
            if (!skiff) continue;
            roster.evict(target, [skiff.x, 0, skiff.z], island.bundle.island.name, skiff.id);
          }
