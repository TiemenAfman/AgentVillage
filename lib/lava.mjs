...
// is hurt through the one door (lib/health.mjs `hurt`, cause `lava`) - today that sends them
// straight back to their square or their skiff, and once the health bar counts it costs
// LAVA_PER_S a second for as long as they stay in it. Plans/vulkaan-in-het-midden.md,
// section 3 and the Besluiten row "Lava aanraken".
//
...
// What a second in lava costs, once hits cost health rather than a trip home. A proposal,
// like the other health numbers (lib/health.mjs): a little under two seconds from whole to
// nothing, so stepping across a stream is survivable and standing in one is not.
...
// A floor this far above the terrain is something built over the lava - a bridge, a deck -
// and whoever stands on it is carried clear. The same floor the guards walk
// (crowd.walk.groundOrDeck), so the two agree about where the planks are.
const DECK_CLEAR = 0.05;
  ...
  function tick() {
    ...
    for (const p of players) {
      ...
      for (const island of hot) {
        ...
        // touching it, and today any touch is a trip home.
