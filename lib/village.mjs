...
import { ADJECTIVES, NOUNS, titleCase, seededName } from './words.mjs';
// Re-exported because it lived here for a long time and half the tree still asks here
// for it. Its home is lib/words.mjs now - beside the two lists it draws from, and
// reachable without importing this file's scanner.
export { seededName };
...
export const MILESTONES = [
  ...
  // The shops of the town's plan (Plans/knus-dorpscentrum.md), in the gaps of the ladder: the
  // bakery on a corner of the square, the rest along the four streets, filling them from the
  // square outwards. `RING_OF` and `STREET_LOTS` in lib/layout.mjs say which lot each takes.
  { id: 'bakery', at: 12, civicType: 'bakery', label: 'The bakery' },
  { id: 'tavern', at: 15, civicType: 'tavern', label: 'The tavern' },
  { id: 'grocer', at: 18, civicType: 'grocer', label: 'The grocer' },
  { id: 'clocktower', at: 20, civicType: 'clocktower', label: 'The clock tower' },
  { id: 'apothecary', at: 22, civicType: 'apothecary', label: 'The apothecary' },
  ...
  { id: 'tailor', at: 28, civicType: 'tailor', label: 'The clothes shop' },
  { id: 'windmill', at: 30, civicType: 'windmill', label: 'The windmill' },
  { id: 'library', at: 33, civicType: 'library', label: 'The library' },
  // A lot of its own rather than a single cell, because the tower's legs span most of a
  // cell and it wants the front path a three by three lot brings with it - see `SMALL` in
  // lib/layout.mjs, which is the list of civics that stand on one cell, and which this is
  // deliberately not on.
  { id: 'watertower', at: 35, civicType: 'watertower', label: 'The water tower' },
  { id: 'tearoom', at: 38, civicType: 'tearoom', label: 'The tea room' },
  { id: 'chapel', at: 40, civicType: 'chapel', label: 'The chapel' },
  { id: 'wandmaker', at: 42, civicType: 'wandmaker', label: 'The wand maker' },
  { id: 'fountain', at: 45, civicType: 'fountain', label: 'The fountain' },
  { id: 'butcher', at: 48, civicType: 'butcher', label: 'The butcher' },
  { id: 'lighthouse', at: 50, civicType: 'lighthouse', label: 'The lighthouse' },
  // The two trades (Plans/zagerij.md, Plans/smidse.md), in the gap between the lighthouse
  // and the statue that the ladder used to leave empty for twenty settlers. They are
  // workshops rather than the town's standing, so they are not handed a lot on the square:
  // `TRADES` in lib/layout.mjs puts them out beyond the shopping streets.
  { id: 'sawmill', at: 55, civicType: 'sawmill', label: 'The sawmill' },
  { id: 'smithy', at: 60, civicType: 'smithy', label: 'The smithy' },
  { id: 'sweetshop', at: 65, civicType: 'sweetshop', label: 'The sweet shop' },
  { id: 'statue', at: 70, civicType: 'statue', label: 'The statue' },
  // The stable (Plans/stal-en-veld.md), the first rung in the twenty-settler stretch between
  // the statue and the bridge, placed by `TRADES` like the sawmill and the smithy: its paddock
  // makes it the bigger yard, and a horse is what a town keeps once it has somewhere to ride
  // to. The bakery that came with it on the animals' branch at 65 is a shop of the square in
  // the town's plan now, at 12 above (Plans/knus-dorpscentrum.md).
  { id: 'stable', at: 75, civicType: 'stable', label: 'The stable' },
  { id: 'cauldron', at: 80, civicType: 'cauldron', label: 'The cauldron maker' },
  // 85 rather than 75, which is the stable's: tests/trades.test.mjs keeps a trade's rung its own.
  { id: 'owlpost', at: 85, civicType: 'owlpost', label: 'The owl post' },
  // The one rung that is not a building. A river never quite severs the island - you can
  // always walk round its head - so a crossing is never forced, and measured on this
  // island no hamlet road has ever wanted one: every project lives on the town's own bank,
  // so `routePath` has never had a route to bend over the water. Cutting the price of a
  // deck does not change that; there is nothing to make cheaper. So the village builds one
  // on purpose instead, and from then on the roads have a crossing to prefer.
  //
  // Ninety, and the number is the town square's own: SQUARE_STEPS in lib/layout.mjs stops
  // widening the plaza at ninety, which is the moment the centre stops growing outward.
  // What the village does next is reach across the water rather than pave more of its own
  // bank. BRIDGE_AT in lib/layout.mjs is the same number and lays the deck.
  { id: 'bridge', at: 90, civicType: 'bridge', label: 'The plank bridge' },
  { id: 'castle', at: 100, civicType: 'castle', label: 'The castle' },
  // The island runs out of buildable land somewhere past three hundred houses, and an
  // island that cannot grow (lib/layout.mjs growStep) stays that size. So at a hundred and fifty the village learns to keep the
  // sea out instead, and the mill that drains the first polder stands on its dike.
  // POLDER_AT in lib/layout.mjs is the same number and reclaims the land.
  { id: 'poldermill', at: 150, civicType: 'poldermill', label: 'The polder mill' },
  // The rung over the mill. Above 150 the ladder used to stop while the village kept
  // growing, and the obvious answer - one more monument on the square - is the wrong one:
  // SQUARE_STEPS in lib/layout.mjs stops widening the square at 90, so by the castle the
  // centre is full and anything else put there is a thing to look at rather than a thing
  // that works. The crane stands on the quay's own waterfront instead, on ground the quay
  // already holds, and swings out over the water a boat ties up in.
  //
  // 165 rather than 175, and the gap is the argument. POLDER_EVERY is 25, so the second
  // polder is drained at 175 - and every dike after the first is stone and timber that has
  // to come ashore. The crane has to be standing before the island asks it to do that, not
  // in the same breath, so it goes in the window between the two polders rather than on
  // either of them.
  { id: 'crane', at: 165, civicType: 'crane', label: 'The harbour crane' },
];
...
// A hamlet the keeper named rather than one a repository founded. `n:` never meets a
// `p:` path, the quay or the outlands. A project in every other respect - it has land, a
// square and a road - but no root to show and no office, because there is no repository
// behind it. The hue is hashed from the id like a project's is from its key.
export function namedDistrict(id, name, outpost = null) {
  return {
    id,
    kind: 'project',
    name: String(name || id.slice(2)).slice(0, 32),
    root: null,
    hue: hash32(id) % 360,
    gitRepo: false,
...
export function buildVillage({
  ...
  // Houses the keeper has given a hamlet of their own - `layout.rehomed`, written by the
  // planner's `rehome` op (lib/plan.mjs, Plans/huis-naar-eigen-wijkje.md).
  rehomed = null,
}) {
  ...
  for (const s of sessions.values()) {
    ...
    settlers.push({
      ...
      source: s.origin === 'codex' ? 'codex' : isCowork ? 'cowork' : (agg ? 'code' : 'hook'),
  ...
  // ---- a hamlet of their own ------------------------------------------------------
  // After the folding, so the keeper's word is the last one: a session the keeper has
  // rehomed lives in the named hamlet whatever folder it worked in. Before the sheds, so
  // its apprentices follow it there the way they follow any master.
  if (rehomed) {
    for (const s of settlers) {
      const r = Object.prototype.hasOwnProperty.call(rehomed, s.id) ? rehomed[s.id] : null;
      if (!r || typeof r.district !== 'string' || !r.district.startsWith('n:')) continue;
      s.district = namedDistrict(r.district, r.name, s.district.outpost);
    }
  }
  ...
  return {
    ...
    dropped,
    milestones,
    // When the nth settler arrived, oldest first. The milestones above are one consumer;
    // the polder ladder is another, and anything else counted in settlers can use it
    // rather than restating how a count becomes a date.
    arrivals: ordered.map((s) => s.startedAt),
