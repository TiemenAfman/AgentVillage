...
//
// The last one is what stops one islander counting twice. An islander with Codex data also
// publishes a Codex neighbour (serve.mjs, codexClient) over a socket of its own, under its
// own id and token, and the fleet has no way to tell that the two sockets are one machine -
// the name on them is chosen by the islander and two people may share one. But that island
// is always hostile (codexBundle sets it, and nothing else can: an islander's own bundle
// never carries it), and no islander's home island ever is. So: islands, minus the hostile
// ones, is islanders. When step 6 retires the per-islander Codex island this rule simply
// stops having anything to exclude.
