...
// ---- the starters -------------------------------------------------------------------
//
// Small islands the sea puts out itself, for a sea that would otherwise be the volcano and
// nothing else whenever the islanders are offline (Plans/starter-eilanden.md). No settlers
// and no agents: a square with a well and tables, a tavern and a town hall, and so an
// innkeeper and a mayor, because lib/crowd.mjs puts a keeper at every building KEEPERS
// knows. A newcomer's island takes the berth of the first free one.
//
// Like the volcano, each follows from its slot number alone, so a restarted sea puts out the
// same ones and still writes nothing down. Unlike the volcano it is an ordinary island in
// every other respect: a number for a seed, the ordinary terrain, nothing hostile.
//
// `size` is the island, `room` the berth it holds: the room a newcomer is founded with by
// default (maxGridSize in config.example.json), so the island that takes it fits where it
// stood. `free` is how many unclaimed ones the sea keeps out.
export const STARTER = Object.freeze({ size: 64, room: 384, free: 3 });

// Sixteen hex digits like every island id (ISLAND_ID), `5ea5` for the sea's, then the slot.
// A real island's id is a sha256 prefix, so landing on one of these is a one in 2^48 event.
export const starterId = (slot) => `5ea5${slot.toString(16).padStart(12, '0')}`;

const STARTER_NAMES = ['Nieuwland', 'Braakland', 'Vrijhaven', 'Eerste Kust', 'Zandhoek', 'Open Oord', 'Stille Hoek', 'Verre Kaap'];
export function starterName(slot) {
  const round = Math.floor(slot / STARTER_NAMES.length);
  return STARTER_NAMES[slot % STARTER_NAMES.length] + (round ? ` ${round + 1}` : '');
}

// Where the town goes: the first spot, spiralling out from the middle, with room for a
// square of five by five, the tavern's lot north of it and the town hall's south, and a row
// of paving between each lot and the square - seven cells across, thirteen deep, all of it
// buildable ground.
function starterTown(terrain) {
  const { size, half } = terrain;
  const fits = (cx, cz) => {
    for (let gz = cz - 6; gz <= cz + 6; gz++) for (let gx = cx - 3; gx <= cx + 3; gx++) {
      if (gx < 0 || gz < 0 || gx >= size || gz >= size || !terrain.isBuildable(gx, gz)) return false;
    }
    return true;
  };
  ...
  for (let r = 0; r < half; r++) {
    for (let dz = -r; dz <= r; dz++) for (let dx = -r; dx <= r; dx++) {
      ...
      if (fits(mid + dx, mid + dz)) return [mid + dx, mid + dz];
    }
  }
  return null;
}

// A starter as a bundle, through parseBundle like the volcano and for the same reason: one
// shape of bundle for a page to know. The seed is tried in steps until the ground has room
// for the town, which on a 64 island is nearly always the first; the steps are fixed, so the
// answer is the same on every start.
export function starterBundle(slot) {
  if (!Number.isInteger(slot) || slot < 0) throw new Error('a starter is numbered from 0');
  for (let k = 0; k < 32; k++) {
    const seed = 50000 + slot * 7919 + k * 104729;
    const terrain = makeTerrain(seed, { size: STARTER.size });
    const at = starterTown(terrain);
    if (!at) continue;
    const [cx, cz] = at;
    const paved = [];
    for (let gz = cz - 3; gz <= cz + 3; gz++) for (let gx = cx - 2; gx <= cx + 2; gx++) paved.push([gx, gz]);
    return parseBundle({
      v: BUNDLE_V,
      island: {
        id: starterId(slot),
        name: starterName(slot),
        keeper: 'the sea',
        seed,
        gridSize: STARTER.size,
        room: STARTER.room,
        terrainHash: terrain.hash,
        foundedAt: null,
        landing: null,
        harbours: [],
        town: { square: [cx - 2, cz - 2], centre: [cx, cz], size: 5, coreR: 2, paved, lots: [] },
        lattice: null,
      },
      grid: { size: STARTER.size },
      buildings: [
        // Its front to the square: rot 2 faces +z, rot 0 faces -z.
        { id: 'civic:tavern', kind: 'civic', civicType: 'tavern', name: 'The tavern', label: 'Tavern', tier: 'civic',
          plot: { gx: cx - 1, gz: cz - 6, w: 3, d: 3, rot: 2 }, door: [cx, cz - 4] },
        { id: 'civic:townhall', kind: 'civic', civicType: 'townhall', name: 'Town Hall', label: 'Town hall', tier: 'civic',
          plot: { gx: cx - 1, gz: cz + 4, w: 3, d: 3, rot: 0 }, door: [cx, cz + 4] },
        { id: 'civic:well', kind: 'civic', civicType: 'well', name: 'The village well', label: 'Well', tier: 'civic',
          plot: { gx: cx, gz: cz, w: 1, d: 1, rot: 0 } },
        { id: 'civic:tables', kind: 'civic', civicType: 'tables', name: 'Tables on the square', label: 'Tables', tier: 'civic',
          plot: { gx: cx + 1, gz: cz + 1, w: 1, d: 1, rot: 0 } },
      ],
    }, { terrainHash: terrain.hash });
  }
  throw new Error(`no seed for starter ${slot} has room for a town`);
}
