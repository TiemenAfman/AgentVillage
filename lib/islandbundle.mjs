...
import { VOLCANO, volcanoTerrain, guardhouseSpec, CODEX, CODEX_TIERS } from '../shared/volcano.mjs';
...
export function parseBundle(obj, { terrainHash = null } = {}) {
  ...
  const actual = terrainHash || makeTerrain(bundle.island.seed, { size, polders, fairway: channel, volcano }).hash;
...
// the guardhouse (shared/volcano.mjs), at a spot worked out from the ground so it is the
// same on every start; the guards who live in it are the crowd's (lib/crowd.mjs), not this
