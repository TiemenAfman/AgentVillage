...
//
// Every mutator takes an options object whose only interesting member is `file`, left over
// from when a visiting island's own scenery was parked in a file of its own under
// data/guests/<id>/ on the host's disk. The sea has since taken over showing a neighbour's
// island (see "Nothing a visitor can reach writes anything" in CLAUDE.md), so nothing calls
// this with anything but the default any more - every caller in serve.mjs and
// tools/island.mjs is unchanged and just gets this island's own props.json.
...
// Measured against the most the island may grow to (islandCap), not the grid it was founded
// on: a tree planted on the ring a growing island gained would otherwise be out at sea.
const reach = (gridSize = null) => (Number(gridSize) || islandCap(loadConfig())) / 2 + 20;
...
// Null for a prop that is not there, rather than a throw: unbuilding something that has
// already gone is not an error, it is the state you asked for.
