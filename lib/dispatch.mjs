...
// project folder, working the ticket the way the jira-ticket-oppakken skill prescribes.
//
...
import { DATA, ensureData } from './paths.mjs';
...
// The trigger phrase the skill listens for, plus the context the agent cannot see.
export function buildPrompt(issue, settler) {
  const lines = [
    `pak ${issue.key} op`,
    '',
    settler.newcomer
      ? `Deze taak is je van het prikbord van Promptholm meegegeven; je bent net op het eiland aangekomen als ${settler.name}.`
      : `Deze taak is je op het prikbord van Promptholm overhandigd, namens kolonist ${settler.name}.`,
    `Volg de skill jira-ticket-oppakken van begin tot eind en koppel aan het einde terug zoals daar beschreven.`,
  ];
  if (issue.summary) lines.push('', `Titel volgens Jira: ${issue.summary}`);
  return lines.join('\n');
}
...
export function buildCommand({ issue, settler, cwd, permissionMode = 'bypassPermissions' }) {
  ...
  args.push('-p', buildPrompt(issue, settler));
