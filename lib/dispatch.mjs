...
import { DATA, ensureData, loadConfig, DEFAULT_DISPATCH } from './paths.mjs';
...
const MAX_LIVE_AGENTS = Number(process.env.PROMPTHOLM_MAX_AGENTS || 4);
...
function claimSlot(name) {
  ...
  if (running.length >= MAX_LIVE_AGENTS) {
    throw new Error(`${running.length} agents are already at work (${running.map((r) => r.name).join(', ')}). Wait for one to finish, or raise PROMPTHOLM_MAX_AGENTS.`);
  }
}
...
// A card on either board: a Jira key, or the number of a GitHub issue.
...
//   }
//
// Placeholders: {key} {number} {summary} {settler} {island}. With no skill configured the
// agent gets a plain instruction to work the ticket and report back.
//
// The island's own board, the one with the GitHub issues of this repository on it, hands
// its cards over with its own wording; it passes that in rather than changing this.
// DEFAULT_DISPATCH moved to lib/paths.mjs, so config.json can be written out with the same
// wording in it that this file falls back to.
...
const FRAMING = {
  nl: {
    ...
    title: (t, where) => `Titel volgens ${where}: ${t}`,
  },
  en: {
    ...
    title: (t, where) => `Title in ${where}: ${t}`,
  },
};

export function buildPrompt(issue, settler, cfg = {}, wording = null) {
  const d = { ...DEFAULT_DISPATCH, ...(cfg.dispatch || {}), ...(wording || {}) };
  const island = cfg.islandName || 'the island';
  const words = FRAMING[d.language] || FRAMING.en;
  const number = issue.number != null ? String(issue.number) : String(issue.key).replace(/^#/, '');
  ...
  const lines = [
    ...
    d.skill ? words.skill(d.skill) : fill(d.plain || words.plain),
  ];
  if (issue.summary) lines.push('', words.title(issue.summary, d.where));
  return lines.join('\n');
}
...
export function buildCommand({ issue, settler, cwd, permissionMode = 'bypassPermissions', wording = null }) {
  ...
  args.push('-p', buildPrompt(issue, settler, loadConfig(), wording));
...
export function found({ cwd, model = null, prompt, permissionMode = 'bypassPermissions' }) {
  ...
  const child = spawn(exe, args, {
    ...
    env: { ...process.env, PROMPTHOLM_FOUNDED: person.sessionId },
  });
...
export function dispatch({ issue, settler, cwd, permissionMode = 'bypassPermissions', dryRun = false, wording = null, source = 'jira' }) {
  if (!issue || !KEY.test(String(issue.key || ''))) throw new Error('that does not look like an issue key');
  if (!cwd || !fs.existsSync(cwd)) throw new Error(`no such folder: ${cwd}`);

  const cmd = buildCommand({ issue, settler, cwd, permissionMode, wording });
  ...
  fs.mkdirSync(AGENT_LOGS, { recursive: true });
  // "#11" is a fine name for a card and a poor one for a file.
  const stem = String(issue.key).replace(/^#/, 'issue-').replace(/[^A-Za-z0-9._-]+/g, '-');
  const logFile = path.join(AGENT_LOGS, `${stem}-${cmd.sessionId.slice(0, 8)}.log`);
  ...
  const child = spawn(exe, cmd.args, {
    ...
    shell,
    env: { ...process.env, PROMPTHOLM_DISPATCH: id },
  });
  child.on('error', (e) => {
    log(`FAILED ${id}: ${e.message}`);
    try { fs.writeSync(out, `\n[promptholm] could not start the agent: ${e.message}\n`); } catch { /* ignore */ }
