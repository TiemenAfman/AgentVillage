...
//
// One pilot per boat, and room for a crew beside them (Plans/lopen-op-de-boot.md): how many
// a boat holds is its craft's (shared/crafts.mjs), the pilot included. Every boat today is a
// Benchy with room for her pilot alone, and no page sends `board` yet, so none of the crew
// rules below has anybody to apply to; they are here so that a bigger boat is a new craft
// and not a new server. The three answers that used to be the reason passengers were out of
// scope: where a body stands is its own deck position (shared/deck.mjs), the pilot letting
// go leaves the crew aboard a boat that runs out its way (`letGo`), and a pilot whose tab
// closes mid-channel frees the tiller and leaves the crew where they stand (`release`).

import { craftOf as defaultCraftOf } from '../shared/crafts.mjs';

// A hull's frame for shared/deck.mjs, from its heading: the one place on the sea that turns a
// yaw into the vector that file works in (it may not call Math.sin itself).
export const frameOf = (b) => ({ x: b.x, z: b.z, fx: Math.sin(b.yaw), fz: Math.cos(b.yaw) });
...
// A skiff: the boat of somebody with no island, and so with no mooring - the app on a
// phone, which starts in the middle of the sea (Plans/eiland-op-android.md). Its id is its
// owner's, `boat:w-<player id>`, so it cannot be claimed under anybody else's name and
// there is at most one per connection: the map cannot grow past the roster however hard a
// socket is poked. It exists from `launch` until its owner leaves (`sink`), and only its
// owner may sail it - a boat everybody may take is one with a mooring to go back to, and
// this one has none; left to a stranger it would be marooned with nobody to fetch it.
export const skiffOf = (playerId) => `boat:w-${playerId}`;
const SKIFF = /^boat:w-/;
// How far from the middle of a hull somebody may be when they step aboard: the Benchy's
// reach from the boat prompt (main.js `r: 2.4`), and plenty for a bigger deck's gangway.
// From a stranger's socket, so it is checked here rather than trusted.
const BOARD_REACH = 3;
// How long the pilot who let go of the tiller may still say where the hull has got to, while
// it runs out the way it had (Plans/lopen-op-de-boot.md, fase 6). Nobody else is simulating
// that boat, and a hull that stopped dead the moment its helm was let go is a crew thrown
// over the bow. Long enough for a full-speed boat to coast to a standstill, and not so long
// that a let-go boat is somebody's for good.
export const COAST_MS = 8000;
...
// `craftOf` says what kind of boat an id is (shared/crafts.mjs); handed in only so a test can
// try the crew rules on a boat with room for more than one, which no island has yet.
export function createBoats({ moorings = [], bound = BOUND, craftOf = defaultCraftOf } = {}) {
  const crewOf = (id) => craftOf(id).crew;
  ...
  // A boat's crew rides along only when it has one, so every message about a boat with
  // nobody but a pilot in it - which is every message there has been - reads exactly as it
  // always did, to an older page and to every test that compares one.
  const state = (id, b) => ({ id, x: b.x, z: b.z, yaw: b.yaw, pilot: b.pilot, ...(b.crew && b.crew.size ? { crew: [...b.crew] } : {}) });
  // Who is aboard: the pilot and the crew beside them. `crew` holds the rest, never the pilot.
  const aboardCount = (b) => (b.crew ? b.crew.size : 0) + (b.pilot ? 1 : 0);
  ...
  function take(id, who) {
    const by = pilot(who);
    if (!by) return null;
    if (SKIFF.test(id) && id !== skiffOf(by)) return null;
    const b = boat(id);
    if (!b) return null;
    if (b.pilot && b.pilot !== by) return state(id, b);
    // A boat with room for a crew is taken from its own deck: step aboard first (`board`),
    // then the helm. A boat that is only its pilot is boarded by taking it, as it always was -
    // unless somebody is already aboard her, because the helm is a place on the deck too.
    const already = b.pilot === by || (!!b.crew && b.crew.has(by));
    if (!already && (crewOf(id) > 1 || aboardCount(b) >= crewOf(id))) return state(id, b);
    if (b.crew) b.crew.delete(by);
    b.pilot = by;
    b.freedAt = null;
    b.coast = null;
    return state(id, b);
  ...
  // Stepping aboard as crew, from `at` - where the sea last had this body in the world. Full
  // is full, and far is far: a hull more than BOARD_REACH away is not one you are standing
  // next to. A skiff is its owner's alone, crew included.
  function board(id, who, at) {
    const by = pilot(who);
    if (!by || !Array.isArray(at)) return null;
    if (SKIFF.test(id) && id !== skiffOf(by)) return null;
    const b = boat(id);
    if (!b) return null;
    if (b.pilot === by || (b.crew && b.crew.has(by))) return state(id, b);
    if (aboardCount(b) >= crewOf(id)) return null;
    const dx = at[0] - b.x, dz = at[1] - b.z;
    if (!(dx * dx + dz * dz <= BOARD_REACH * BOARD_REACH)) return null;
    // Off any other boat first: one body stands on one deck.
    for (const [other, ob] of boats) if (other !== id && ob.crew) ob.crew.delete(by);
    if (!b.crew) b.crew = new Set();
    b.crew.add(by);
    b.freedAt = null;
    return state(id, b);
  }

  // Stepping off - onto a quay, or over the side. A pilot stepping off lets go of the tiller
  // on the way, exactly as `drop` does; the crew stays where it stands.
  function leave(id, who) {
    const by = pilot(who);
    const b = by && boats.get(id);
    if (!b) return null;
    if (b.pilot === by) { b.pilot = null; b.coast = null; }
    else if (!(b.crew && b.crew.delete(by))) return null;
    if (!aboardCount(b)) b.freedAt = Date.now();
    return state(id, b);
  }

  // Letting go of the tiller while staying aboard (fase 6): the pilot becomes crew, the helm
  // is free for anybody else on the deck, and for COAST_MS the one who let go may still move
  // the hull while it runs out its way - see `moved`.
  function letGo(id, who, now = Date.now()) {
    const by = pilot(who);
    const b = by && boats.get(id);
    ...
    if (!b.crew) b.crew = new Set();
    b.crew.add(by);
    b.coast = { by, until: now + COAST_MS };
    return state(id, b);
  }

  // Whether somebody is on this boat, at the helm or on deck. What a pose that says it is
  // standing on a deck is checked against (lib/players.mjs).
  function aboard(id, who) {
    const b = boats.get(id);
    return !!b && (b.pilot === who || (!!b.crew && b.crew.has(who)));
  }
  // Where a boat is, for the arithmetic of standing on it; null for one never touched.
  function pose(id) {
    const b = boats.get(id);
    return b ? { x: b.x, z: b.z, yaw: b.yaw } : null;
  }
  ...
  function moved(id, by, x, z, yaw, now = Date.now()) {
    const who = pilot(by);
    const b = who && boats.get(id);
    // The pilot, or whoever let go of the tiller a moment ago while the hull still has way
    // on it (`letGo`) - and only until somebody else takes it.
    const coasting = !!b && !b.pilot && !!b.coast && b.coast.by === who && now < b.coast.until;
    if (!b || (b.pilot !== who && !coasting)) return null;
  ...
  function release(playerId) {
    ...
    for (const [id, b] of boats) {
      const crewed = !!b.crew && b.crew.delete(who);
      if (b.coast && b.coast.by === who) b.coast = null;
      if (b.pilot !== who && !crewed) continue;
      if (b.pilot === who) b.pilot = null;
      if (!aboardCount(b)) b.freedAt = Date.now();
      out.push(state(id, b));
    }
    return out;
  ...
  function driftHome(now = Date.now()) {
    const out = [];
    for (const [id, b] of boats) {
      // Not with anybody aboard: a crew waiting on a boat nobody is steering is standing on
      // it, and a boat that sailed home under them would be a jump across the sea.
      if (b.pilot || aboardCount(b) || b.freedAt == null || now - b.freedAt < IDLE_HOME_MS) continue;
      const at = home.get(id);
      if (!at) continue;
      b.freedAt = null;
  ...
  // Putting a skiff in the water, with its owner at the tiller, wherever they are. A second
  // launch is the same boat again - a page that reconnects gets its own hull back rather
  // than a second one - and moves it to where that page now says it is.
  function launch(who, x, z, yaw) {
    const by = pilot(who);
    if (!by) return null;
    const nx = num(x, bound), nz = num(z, bound), ny = num(yaw === undefined ? 0 : yaw, YAW_LIMIT);
    if (nx === null || nz === null || ny === null) return null;
    const id = skiffOf(by);
    boats.set(id, { x: nx, z: nz, yaw: ny, pilot: by, freedAt: null });
    return state(id, boats.get(id));
  }

  // Its owner has gone, so it goes too: there is no mooring to drift it back to and nobody
  // else may sail it. Handed back in the shape moor() hands a boat whose island left, so
  // every page takes it out of the water through the path it already has.
  function sink(who) {
    const by = pilot(who);
    const id = by && skiffOf(by);
    const b = id && boats.get(id);
    if (!b) return [];
    boats.delete(id);
    return [{ id, x: b.x, z: b.z, yaw: b.yaw, pilot: null, gone: true }];
  }

  return { snapshot, take, drop, moved, release, forget, moor, driftHome, launch, sink, board, leave, letGo, aboard, pose, count: () => boats.size };
}
