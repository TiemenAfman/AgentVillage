...
export function createFleet({ now = () => Date.now(), gap = SEA_GAP, max = MAX_ISLANDS } = {}) {
  ...
  function publish(id, raw, { token = null } = {}) {
    ...
    const island = {
      ...
      // Out of the bundle's own districts where it has a quay - the kade a village built is
      // its harbour, and deriving off the landing instead moored the boat at a second pier
      // somewhere else on the coast. The bundle carries the planks, so the sea reaches the
      // same answer as the browser that sent them.
      mooring: mooringFor(id, terrain, bundle.island.landing, origin, planksOf(bundle)),
    };
