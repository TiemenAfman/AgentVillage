...
import { parseBundle, parseParcel, volcanoBundle, starterBundle, STARTER, ISLAND_ID, GRID_MAX } from './islandbundle.mjs';
...
// sink an island under the feet of somebody standing on it - and neither may a keeper
// going to bed. It was 45 s, which covered the first three; but a phone has no island of
// its own (Plans/eiland-op-android.md), so a sea whose islanders were all offline was the
// volcano and nothing else to it. Three days carries a weekend: the island stays quiet
// (goneQuiet - drawn, walked, its keepers at their posts, no new shape) until its islander
// is back or the grace runs out. The price is a ghost holding one of MAX_ISLANDS' places
// that long, and an islander with a new token being refused as `claimed` until then.
...
export function createFleet({ now = () => Date.now(), gap = SEA_GAP, max = MAX_ISLANDS } = {}) {
  ...
  // What each island holds of the sea: its `reach`, which is the size it may grow to rather
  // than the size it is (bundle.island.room, Plans/eiland-laten-groeien.md). Berths are laid
  // out on that, so an island growing into its own room never reaches a neighbour and never
  // has to be given another berth - "an island never moves" holds while it grows.
  const placed = () => [...islands.values()].map((i) => ({ half: i.reach ?? i.half, origin: i.origin }));
  const reachOf = (bundle, terrain) => Math.max(terrain.half, Math.min(GRID_MAX, Number(bundle.island.room) || 0) / 2);
  // How many islands belong to somebody.
  const theirs = () => [...islands.values()].filter((i) => !i.sea).length;
  // What `max` is measured against: everybody's, and the starters holding a berth for a
  // newcomer (raiseStarters). A starter is drawn like any island, and the cap is the draw
  // budget; a newcomer taking one reuses its place rather than adding one.
  const places = () => [...islands.values()].filter((i) => !i.sea || i.starter).length;
  // The unclaimed starters, first slot first: the order a newcomer takes them in.
  const starters = () => [...islands.values()].filter((i) => i.starter).sort((a, b) => a.slot - b.slot);
  // The next slot to raise. Counted up in memory and never reused, so a starter that was
  // taken never comes back under its old id to a page that just saw it replaced; a restarted
  // sea counts from 0 again and raises the same ones.
  let nextSlot = 0;
  ...
  function row(i) {
    return {
      ...
      // The half it holds of the sea (bundle.island.room), which berths are laid out on: a
      // starter is 64 across and holds 384. A page placing a berth of its own - the phone's
      // open water (standaloneHome in web/js/main.js) - has to keep out of it too.
      reach: i.reach ?? i.half,
      polders: i.bundle.polders || [],
      // A grown island's silhouette is its grown coast: horizon.js builds it from this row.
      ...(i.bundle.grow ? { grow: i.bundle.grow } : {}),
  ...
  function publish(id, raw, { token = null, live = true } = {}) {
    ...
    // which a polder or a re-founding can do. Only a newcomer is given one: the berth of the
    // first starter it fits in (Plans/starter-eilanden.md), which goes to make room, or else
    // the next free one. A starter holds STARTER.room, so every newcomer on the default room
    // fits; one asking for more gets a berth of its own and the starters stay.
    const reach = reachOf(bundle, terrain);
    const took = had ? null : starters().find((s) => reach <= s.reach) || null;
    if (!had && !took && places() >= max) throw refuse('this sea is full', 503);
    let origin = had ? had.origin : took ? took.origin : nextOrigin(placed(), reach, gap);
    if (!origin) throw refuse('this sea is full', 503);
    if (had && (had.reach ?? had.half) !== reach) {
      // It asks for more room than it had (or less). Keep the berth if it still fits; ask for
      // another only if it does not - which, with the room reserved, is a keeper raising the
      // island size past what the neighbours left, not the island growing into it.
      const others = [...islands.values()].filter((i) => i.id !== id).map((i) => ({ half: i.reach ?? i.half, origin: i.origin }));
      const mine = { half: reach, origin };
      ...
      if (!clear) origin = nextOrigin(others, reach, gap);
      if (!origin) throw refuse('this sea is full', 503);
    }
    ...
    // The starter this island replaced, for the sea to take off the map and send its
    // visitors back from. Only on the record the claim made: the next publish builds a
    // new one without it.
    if (took) {
      islands.delete(took.id);
      island.took = took;
    }
  ...
  function raiseVolcano() {
    ...
    for (const i of islands.values()) {
      const need = (i.reach ?? i.half) + terrain.half + gap;
  ...
  // Put out starters until STARTER.free of them are unclaimed, or the sea has no place for
  // another. Like the volcano they are the sea's (`sea: true`: no token, never swept, and
  // `claim`, `patch` and `vouch` refuse them), with `starter: true` for the one thing that
  // differs - a newcomer's publish may take one's berth. Each from its slot alone
  // (starterBundle), berthed with nextOrigin like anybody, so a restarted sea puts out the
  // same ones in the same places. Returns the ones it raised, for the sea to give a crowd
  // and announce; empty when there was nothing to do.
  //
  // Raised after the volcano, which has to hold the origin before nextOrigin rings
  // anybody round it.
  function raiseStarters() {
    const raised = [];
    while (starters().length < STARTER.free && places() < max) {
      const bundle = starterBundle(nextSlot);
      const terrain = makeTerrain(bundle.island.seed, { size: bundle.island.gridSize });
      const reach = bundle.island.room / 2;
      const origin = nextOrigin(placed(), reach, gap);
      if (!origin) break;
      const island = {
        id: bundle.island.id,
        slot: nextSlot,
  ...
  return {
    publish, patch, vouch, claim, goneQuiet, expired, drop, raiseVolcano, raiseStarters, furnishVolcano,
