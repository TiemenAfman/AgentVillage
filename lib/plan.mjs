...
const OPS = new Set(['move', 'zone', 'polder', 'unpolder', 'parcel', 'rehome']);
...
export function applyPlan(layout, model, plan, { seed, size, now = Date.now() }) {
  ...
  return { ok: verdicts.every((v) => v.ok), verdicts, touched, reroute, remodel, pruned };
}
...
export function runPlan(layout, model, plan, { seed, size, dryRun = false, layoutFile = null, placementsFile = null, now = Date.now(), remodel = null }) {
  const t0 = Date.now();
  ...
  let placed = null;
  if (a.ok) placed = placeAllToFixedPoint(trial, modelFor(trial, a), { seed, size });
  ...
  const final = placeAllToFixedPoint(layout, after, { seed, size });
