...
import {
  ...
  polderCandidate, touchesShore, polderFromSupers, digPolder, fairwayHeld, zoneCells,
  NONE, TOWN, TOWN_CORE_R, BELT,
} from './layout.mjs';
...
const OPS = new Set(['move', 'zone', 'polder']);
...
export function applyPlan(layout, model, plan, { seed, size, now = Date.now() }) {
  ...
  for (const [i, op] of plan.ops.entries()) {
    ...
    try {
      const r = op.op === 'move' ? opMove(layout, op, ctx, v) : op.op === 'polder' ? opPolder(layout, op, ctx, v) : opZone(layout, op, ctx, v);
...
function opPolder(layout, op, ctx, v) {
  ...
  for (const [i, j] of op.supers) {
    if (!superComplete(lat, size, i, j)) throw new Error(`super-cell [${i}, ${j}] is not wholly on the island`);
    if (!polderCandidate(terrain, lat, i, j, keep)) {
      const [gx, gz] = blockOf(lat, i, j);
      let land = 0, deep = 0, kept = 0;
      for (let z = 0; z < lat.pitch; z++) {
        for (let x = 0; x < lat.pitch; x++) {
          if (!terrain.isWater(gx + x, gz + z)) land++;
          else if (keep.has(ckey([gx + x, gz + z]))) kept++;
          else if (terrain.heightAt(gx + x, gz + z) < -1.2) deep++;
        }
      }
      throw new Error(land ? `[${i}, ${j}] is already land` : kept ? `[${i}, ${j}] is the fairway, or zoned to stay water` : deep ? `[${i}, ${j}] is too deep for a dike` : `[${i}, ${j}] cannot be reclaimed`);
    }
    if (touchesShore(terrain, lat, i, j)) shore = true;
  }
  ...
  if (p.road.length <= 1) v.notes.push('no dry road reached the mainland; the causeway is a stub');
...
function opMove(layout, op, ctx, v) {
  ...
  for (const m of lobes) {
    for (const [i, j] of m.lobe.cells) {
      ...
      if (sup.eligible(t[0], t[1], m.k, m.allowBeach)) continue;
      if (!sup.usable(t[0], t[1], m.allowBeach)) {
        ...
        refuse(held ? `[${t[0]}, ${t[1]}] is held ground (a dike, a causeway or a zone)`
          : water ? `[${t[0]}, ${t[1]}] is water` : `[${t[0]}, ${t[1]}] is too steep to build on`);
      }
      const who = ctx.owner(t[0], t[1]);
      if (who) refuse(`[${t[0]}, ${t[1]}] is ${who}'s land`);
      ...
      refuse(`[${t[0]}, ${t[1]}] is within the belt of ${near || 'a neighbour'}'s land`);
    }
  }
...
export function runPlan(layout, model, plan, { seed, size, dryRun = false, layoutFile = null, placementsFile = null, now = Date.now() }) {
  ...
  const ok = a.ok && !!placed && placed.stable && diff.plots.otherMoved.length === 0;
  const out = {
