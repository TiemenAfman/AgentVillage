...
import {
  ...
  growStep, nextCoast, pruneUnreachable,
} from './layout.mjs';
...
function opGrow(layout, op, ctx, v) {
  if (!layout.grow) throw new Error('this island was founded on its whole grid, and cannot grow yet');
  const before = new Set(Object.keys(layout.plots));
  const r = nextCoast(layout.grow, ctx.size);
  const ground = growStep(layout, { seed: ctx.seed, size: ctx.size });
  if (!ground) throw new Error(`the island has reached the edge of its grid: a coast of ${r} does not fit on ${ctx.size}`);
  ctx.terrain = ground;
  ctx.dirty = true;
