...
import {
  ...
  growStep, nextCoast, pruneUnreachable, canGrowFurther,
} from './layout.mjs';
...
const OPS = new Set(['move', 'zone', 'polder', 'unpolder', 'parcel', 'road', 'grow']);
...
function opGrow(layout, op, ctx, v) {
  if (!canGrowFurther(layout, ctx.cap)) {
    throw new Error(layout.grow ? 'the island has reached the most its grid may be (gridSize in config.json)'
      : 'this island was founded on its whole grid; raise gridSize in config.json to give it room to grow');
  }
...
export function runPlan(layout, model, plan, { seed, size, cap = size, dryRun = false, layoutFile = null, placementsFile = null, now = Date.now() }) {
  const t0 = Date.now();
  const before = clone(layout);
  const trial = clone(layout);
  const a = applyPlan(trial, model, plan, { seed, size, cap, now });
  let placed = null;
  if (a.ok) placed = placeAllToFixedPoint(trial, model, { seed, size: trial.size, cap });
  ...
  const were = new Set(stranded(before, before.size || size));
  diff.stranded = placed ? stranded(trial, trial.size).filter((id) => !were.has(id)) : [];
  ...
  const final = placeAllToFixedPoint(layout, model, { seed, size: layout.size, cap });
