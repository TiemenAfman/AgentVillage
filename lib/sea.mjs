...
import { nightAt, borrelAt } from '../shared/daylight.mjs';
...
export function createSea({
  ...
  const beat = setInterval(() => {
    ...
    // On the world's own clock: how dark it is, and whether it is Friday borrel. This was
    // `crowds.tick(0)` from the day the sea took the walking over until 23 September -
    // noon for ever, and a borrel nobody rang (shared/daylight.mjs has the story). The hour
    // and the weekday are the sea clock's (SEA_TZ, lib/seaclock.mjs) through worldTime, the
    // same reading every page makes of the welcome, so the tables come out on the screen at
    // the moment the settlers walk to them.
    const wt = worldTime(now(), clock.offset());
    crowds.setGather(borrelAt(wt.weekday, wt.hour));
