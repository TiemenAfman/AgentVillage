...
export function createSea({
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (p === '/health') return json(res, 200, { ok: true, sea: name, v: SEA_V, keyed: !!key, islands: fleet.count(), players: sockets.size, settlers: crowds.figures(), ticks: crowds.ticks(), gathering: crowds.gathering() ? crowds.gathering().id : null });
