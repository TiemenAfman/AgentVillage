...
export function createSea({
  ...
  const beat = setInterval(() => {
    ...
    // On the world's own clock: how dark it is, and whether the village is due on the square
    // (shared/daylight.mjs GATHERINGS). This was `crowds.tick(0)` from the day the sea took the
    // walking over until 23 September - noon for ever, and a bell nobody rang. The hour and the
    // weekday are the sea clock's (SEA_TZ, lib/seaclock.mjs) through worldTime, the same
    // reading every page makes of the welcome, so the tables come out on the screen at the
    // moment the settlers walk to them.
