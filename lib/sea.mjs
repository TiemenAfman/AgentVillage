...
import { islandClock, nightAt, borrelAt } from '../shared/daylight.mjs';
...
export function createSea({
  ...
  const beat = setInterval(() => {
    ...
    // On the island's own clock: how dark it is, and whether it is Friday borrel. This was
    // `crowds.tick(0)` from the day the sea took the walking over until 23 September -
    // noon for ever, and a borrel nobody rang (shared/daylight.mjs has the story).
    const clock = islandClock(now());
    crowds.setGather(borrelAt(clock.day, clock.hour));
