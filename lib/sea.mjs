...
import { createHostility, afoot, pilotsOf } from './hostility.mjs';
...
import { createHerds } from './animal-crowd.mjs';
import { parseAnimals, MAX_ANIMAL_BYTES } from './animalbundle.mjs';
...
export function createSea({
  ...
  volcano = true,
  // Whether to put out the starters (Plans/starter-eilanden.md): small islands of the sea's
  // own with a square, a tavern and a town hall, so a sea with every islander offline is
  // not the volcano alone. On for every real sea; a test about berths can say no.
  starters = true,
  ...
  // Every island's story animals (lib/animal-crowd.mjs, docs/animals-wire.md): who they are is
  // their islander's and arrives through POST /island/:id/animals; where they are is this
  // sea's, stepped on the beat below and said as `af`. Apart from the crowds on purpose - a
  // republish rebuilds a crowd and must not rebuild a herd - and apart from the fleet's
  // bundle, so a hen changing her mind moves no `rev` and costs nobody a coastline.
  const herds = createHerds({ fleet, now });

  // An errand finished, told to the one socket that asked for it: the island's own islander,
  // on the island's own token. Never broadcast - a `done` is the islander's cue to write a
  // line in its journal, and a page or another islander has no business prompting that. A
  // done with nobody on the line is not kept: the islander posts every pending action again
  // on its next welcome, and the herd answers those out of what it remembers finishing.
  function tellDone() {
    for (const c of herds.completions()) {
      const island = fleet.get(c.island);
      if (!island || !island.token) continue;
      const text = JSON.stringify({ t: 'animal', a: 'done', i: c.island, action: c.action, gen: c.gen });
      for (const s of sockets) {
        if (s.joined && s.as === 'islander' && s.island === c.island && s.token === island.token) {
          try { s.send(text); } catch { /* going away */ }
        }
      }
    }
  }
  ...
  // Then the starters, on the ring round it. Nobody is here yet to be told.
  if (starters) for (const s of fleet.raiseStarters()) crowds.join(s);
  ...
  // A newcomer has taken a starter's berth (lib/fleet.mjs publish). The starter goes off the
  // map, and whoever is walking on it is sent back - to their skiff, or their own square -
  // since the ground under them is about to be another island's, and standing still could
  // leave them in the sea or inside a house. `gone` goes out before the newcomer's `joined`,
  // so a page never holds two islands on one berth.
  function retireStarter(took) {
    crowds.leave(took.id);
    const pilots = pilotsOf(roster);
    const [ox, oz] = took.origin, edge = took.half + 1;
    for (const p of roster.all()) {
      if (!afoot(p, pilots) || Math.abs(p.x - ox) > edge || Math.abs(p.z - oz) > edge) continue;
      const back = health.refuge(p);
      if (back) roster.evict(p, back.at, took.bundle.island.name, back.boat);
    }
    broadcast({ t: 'island', a: 'gone', id: took.id });
    log(`${took.id} (${took.bundle.island.name}) made way`);
  }

  // Put out starters until there are enough unclaimed again, each announced the way a
  // published island is. Does nothing when there are enough, or no place for another.
  function topUpStarters() {
    if (!starters) return;
    for (const s of fleet.raiseStarters()) {
      const crowd = crowds.join(s);
      broadcast({ t: 'island', a: 'joined', ...fleet.row(s) });
      broadcast({ t: 'fr', i: s.id, ids: crowdRoster(crowd) });
      log(`${s.id} (${s.bundle.island.name}) is put out at ${s.origin.join(',')}`);
    }
  }
  ...
  function onMessage(conn, text) {
    ...
    if (m.t === 'join') {
      ...
      // And every island's animals, for the same reason: who they are, then where each of
      // them is, all at once - `mark: false`, because one socket being told everything is not
      // the beat telling everybody, and the next beat's changed rows are measured against
      // what everybody else was last sent.
      for (const id of herds.ids()) {
        if (!fleet.has(id)) continue;
        send(conn, { t: 'herd', i: id, ...herds.publicOf(id) });
        const r = herds.encode(id, { all: true, mark: false });
        if (r.length) send(conn, { t: 'af', i: id, r });
      }
      return;
    }
  }
  ...
  const server = http.createServer(async (req, res) => {
    ...
    // The island's story animals (docs/animals-wire.md): who they are and what their islander
    // would like each of them to do, for this sea to walk (lib/animal-crowd.mjs). A door of its
    // own for the parcel door's reason - a hen that changes her mind is not a new island, and
    // must not move `rev` or rebuild the crowd - and asked in the Codex door's order: the key,
    // then the island's own claim (fleet.vouch), then the body, strictly and against the
    // island's own half. A refusal changes nothing that was there.
    if (p.startsWith('/island/') && p.endsWith('/animals') && req.method === 'POST') {
      const id = decodeURIComponent(p.slice('/island/'.length, -'/animals'.length));
      if (!keyOpens(req)) return json(res, 403, { error: 'key' });
      try {
        const body = await readBody(req, MAX_ANIMAL_BYTES);
        const token = String(req.headers['x-island-token'] || '') || null;
        const island = fleet.vouch(id, token);
        const herd = parseAnimals(body, { half: island.half });
        const r = herds.set(id, herd);
        // Who the indices mean before anybody is told where they are: a changed list may have
        // given an index to somebody else, so every row follows it rather than only the rows
        // the beat would have found changed.
        if (r.changed) {
          broadcast({ t: 'herd', i: id, ...herds.publicOf(id) });
          const rows = herds.encode(id, { all: true });
          if (rows.length) broadcast({ t: 'af', i: id, r: rows });
        }
        // An errand this sea already finished, posted again after a reconnect, is answered
        // now rather than on the next beat.
        tellDone();
        return json(res, 200, { ok: true, animals: herd.animals.length, actions: herd.actions.length, traces: herd.traces.length });
      } catch (e) {
        return json(res, e.status || 400, { error: String(e.message || e) }, { last: !!e.last });
      }
    }

    // And what anybody may know of them: the public state as last posted, without the errands.
    // An island with no herd has no animals - an empty herd, not a 404, which is kept for an
    // island this sea has never heard of.
    if (p.startsWith('/island/') && p.endsWith('/animals') && req.method === 'GET') {
      const id = decodeURIComponent(p.slice('/island/'.length, -'/animals'.length));
      if (!fleet.has(id)) return json(res, 404, { error: 'no such island here' });
      return json(res, 200, herds.publicOf(id) || { seq: 0, animals: [], traces: [] });
    }
    ...
    if (p.startsWith('/island/')) {
      ...
      if (req.method === 'POST') {
        ...
        try {
          ...
          if (island.took) retireStarter(island.took);
          ...
          // Its animals, unlike its settlers, are not rebuilt: they keep their feet and get the
          // new ground and walls under them (lib/animal-crowd.mjs). Now rather than on the next
          // beat, so an animals POST that lands in between plans on the island as it is.
          herds.reground(island.id);
    ...
    json(res, 404, {
      error: `no route ${p} - the sea has: /, /world, /island/:id, /island/:id/parcel, /island/:id/codex, /island/:id/animals, /health, /update and /ws`,
      asked: p,
    });
  });
  ...
  // How often every animal's row goes out, changed or not: two seconds (docs/animals-wire.md),
  // rounded to a whole number of walker beats so the full send lands on a beat that sends
  // animals at all.
  const HERD_ALL_EVERY = MOVERS_EVERY * Math.max(1, Math.round(2000 / (Math.max(1, tickMs) * MOVERS_EVERY)));
  let slice = 0;
  let beats = 0;
  ...
  const beat = setInterval(() => {
    ...
    // The animals, after the crowds: stepped, then said. Rows that changed at the walker rate
    // (MOVERS_EVERY), and every row every HERD_ALL_EVERY beats - about two seconds - so a page
    // that missed a message, or whose herd was redrawn, is never more than that out. Six
    // animals an island make this a few hundred bytes a second however big the village is.
    herds.tick();
    if (beats % MOVERS_EVERY === 0) {
      const all = beats % HERD_ALL_EVERY === 0;
      for (const id of herds.ids()) {
        const r = herds.encode(id, { all });
        if (r.length) broadcast({ t: 'af', i: id, r });
      }
    }
    tellDone();
  ...
  function sweepOnce() {
    for (const id of fleet.expired(GRACE_MS)) {
      ...
      residents.drop(id);
      // And its animals. Said as an empty herd - its own `t`, which a page from before the
      // animals ignores - so a page that drew them takes them away with the coast.
      if (herds.drop(id)) broadcast({ t: 'herd', i: id, seq: 0, animals: [], traces: [] });
      remoor();
      broadcast({ t: 'island', a: 'gone', id });
      log(`${id} has gone home`);
    }
    // A berth given back may be the place a starter was waiting for on a full sea.
    topUpStarters();
  }
