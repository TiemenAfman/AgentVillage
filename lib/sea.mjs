...
export function createSea({
  ...
  function welcome(conn) {
    return {
      ...
      // see one island in daylight and the other at night, in the same frame.
      now: now(),
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (p === '/health') return json(res, 200, { ok: true, sea: name, v: SEA_V, islands: fleet.count(), players: sockets.size });
  ...
  const beat = setInterval(() => roster.tick(), tickMs);
