...
import { islandClock, nightAt, gatheringAt } from '../shared/daylight.mjs';
...
export function createSea({
  ...
  const hostility = createHostility({ fleet, crowds, roster, now });
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (p === '/health') return json(res, 200, { ok: true, sea: name, v: SEA_V, keyed: !!key, islands: fleet.count(), players: roster.count(), settlers: crowds.figures(), ticks: crowds.ticks(), gathering: rang ? rang.id : null });
  ...
  const beat = setInterval(() => {
    ...
    // On the island's own clock: how dark it is, and whether the village is due on the
    ...
    const bell = gatheringAt(clock.day, clock.hour);
    crowds.setGather(!!bell);
    crowds.tick(nightAt(clock.hour));
    hostility.tick();
