...
export function createSea({
  ...
  // the guards below now, lava and other players later. See lib/health.mjs.
  const health = createHealth({ fleet, roster, now });
  const hostility = createHostility({ fleet, crowds, roster, health, now });
  ...
  // - an empty one for now, the guards come later - so a joiner is sent its (empty) roster
  // and the hostility tick finds it where it looks for every hostile island.
  if (volcano) crowds.join(fleet.raiseVolcano());
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (p === '/health') return json(res, 200, { ok: true, sea: name, v: SEA_V, keyed: !!key, islands: fleet.count(), players: roster.walking(), settlers: crowds.figures(), ticks: crowds.ticks() });
