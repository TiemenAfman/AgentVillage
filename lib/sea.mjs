...
import { encodeCrowd, encodeRides, encodeHeld, encodeCarry, crowdRoster, sliceCount, walkerEvery } from '../shared/settlerwire.mjs';
...
export function createSea({
  ...
  function onMessage(conn, text) {
    ...
    if (m.t === 'join') {
      ...
      for (const crowd of crowds.all()) {
        ...
        // And who is carrying gold home from the pit, for the same reason: empty or not,
        // it replaces whatever an earlier sea told this page.
        send(conn, { t: 'fc', i: crowd.id, c: encodeCarry(crowd) });
      }
      return;
    }
  }
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (p === '/health') return json(res, 200, { ok: true, sea: name, v: SEA_V, version: build.version || null, commit: build.commit || null, keyed: !!key, islands: fleet.count(), players: roster.walking(), settlers: crowds.figures(), ticks: crowds.ticks() });
  ...
  // The same, for who is carrying a bar home from the gold pit (encodeCarry,
  // Plans/goudkuil.md): worked out every beat, sent when it changes.
  const carrySaid = new Map();
  ...
  const beat = setInterval(() => {
    ...
    for (const crowd of crowds.all()) {
      ...
      const c = encodeCarry(crowd);
      const carried = c.join(',');
      if (carried !== (carrySaid.get(crowd.id) ?? '')) {
        broadcast({ t: 'fc', i: crowd.id, c });
        carrySaid.set(crowd.id, carried);
      }
    }
