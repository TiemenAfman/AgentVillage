...
export function createSea({
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (p === '/update' && req.method === 'POST') {
      if (key) {
        const said = String(req.headers['x-sea-key'] || '');
        if (said !== key) return json(res, 403, { error: 'that is not the key to this sea' });
      }
    ...
    if (p === '/health') return json(res, 200, { ok: true, sea: name, v: SEA_V, keyed: !!key, islands: fleet.count(), players: sockets.size, settlers: crowds.figures(), ticks: crowds.ticks() });
