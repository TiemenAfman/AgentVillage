...
import { createBoats, frameOf } from './boats.mjs';
import { craftOf as defaultCraftOf } from '../shared/crafts.mjs';
import { toWorld, clampToDeck } from '../shared/deck.mjs';
...
// the player (web/js/bicycle.js, Plans/fiets.md): relayed only, for peers.js to draw. LYING,
// CROUCHING and SITTING are the same kind of word (Plans/andere-spelers-zoals-jij.md): how
// somebody else's body is to be drawn, and nothing the sea acts on. Anything above these is
// dropped here, so a client cannot smuggle a flag the next version might give a meaning to.
export const POSE = Object.freeze({
  MOVING: 1, SWIMMING: 2, RUNNING: 4, AIRBORNE: 8, BLOCKING: 16, SHIELD_LEFT: 32, SHIELD_RIGHT: 64, RIDING: 128,
  LYING: 256, CROUCHING: 512, SITTING: 1024,
});
const POSE_MASK = 2047;
...
// Somebody's own look (web/js/avatar.js - Plans/andere-spelers-zoals-jij.md), checked for its
// shape and nothing else: a colour is a whole number in 0..0xffffff, a hat shape and what a
// hand holds are short slugs, the four pieces of kit are yes or no. What a slug may name is
// the wardrobe's business - the page that draws it runs the look through normalizeAvatar,
// which is the one whitelist of it, and this side may not import web/. What this is for is
// that a stranger's socket cannot hang anything else on a player that every page is sent.
const LOOK_SLUG = /^[a-z][a-z0-9-]{0,15}$/;
const LOOK_KIT = ['backpack', 'chestplate', 'leggings', 'boots'];
function lookOf(m) {
  const colour = (v) => (Number.isInteger(v) && v >= 0 && v <= 0xffffff ? v : null);
  const slug = (v) => (typeof v === 'string' && LOOK_SLUG.test(v) ? v : null);
  const e = m.equip && typeof m.equip === 'object' && !Array.isArray(m.equip) ? m.equip : {};
  const equip = { leftHandItem: slug(e.leftHandItem), rightHandItem: slug(e.rightHandItem) };
  for (const k of LOOK_KIT) if (typeof e[k] === 'boolean') equip[k] = e[k];
  return { skin: colour(m.skin), tunic: colour(m.tunic), trim: colour(m.trim), hat: colour(m.hat), hatShape: slug(m.hatShape), equip };
}
// Which hand a swing or a drink was, as classic-avatar.js names them; anything else is none.
const sideOf = (v) => (v === 'leftArm' || v === 'rightArm' ? v : null);

// How far from the middle of a hull a deck position may claim to be, and how high above its
// planking: the ceilings on `d` before shared/deck.mjs holds it to the deck itself.
const DECK_REACH = 8;
const DECK_HEIGHT = 4;
...
export function createRoster({
  ...
  swing = () => false,
  // What kind of boat an id is, for the size of its crew and the shape of its deck
  // (shared/crafts.mjs). An option only so a test can put a crew on a boat bigger than any
  // island has yet.
  craftOf = defaultCraftOf,
  log = () => {},
} = {}) {
  ...
  const boats = createBoats({ moorings, bound, craftOf });
  let lastRoster = 0;
  ...
  const identity = (p) => ({ id: p.id, name: p.name, style: p.style, keeper: p.keeper, island: p.island || null, look: p.look || null });
  ...
  function message(conn, text) {
    ...
    // What somebody looks like, from their own wardrobe. Said when they connect and when
    // they change it, and handed on in their identity - a join for a known id is how every
    // page already hears that somebody changed (see hello above).
    if (m.t === 'look') {
      const look = lookOf(m);
      if (JSON.stringify(look) === JSON.stringify(p.look || null)) return;
      p.look = look;
      sendAll({ t: 'join', p: identity(p) });
      return;
    }
    ...
    if (m.t === 'p') {
      ...
      p.walking = true;
      p.posed = true;
      // Standing on a deck (Plans/lopen-op-de-boot.md): `on` names the boat and `d` is
      // [x, y, z, yaw] on it, in its own frame (shared/deck.mjs). Taken only from somebody that
      // boat has aboard, held to its planks, and turned into the world here from where the sea
      // has the hull - so the guards, the lava and a page from before this see a passenger
      // where the boat is, and not where the passenger's own screen last drew the boat.
      p.deck = null;
      if (typeof m.on === 'string' && Array.isArray(m.d) && m.d.length === 4 && boats.aboard(m.on, p.id)) {
        const hull = boats.pose(m.on);
        const lx = num(m.d[0], DECK_REACH), ly = num(m.d[1], DECK_HEIGHT), lz = num(m.d[2], DECK_REACH), lyaw = num(m.d[3], 1e4);
        if (hull && lx !== null && ly !== null && lz !== null && lyaw !== null) {
          const [cx, cz] = clampToDeck(craftOf(m.on), lx, lz);
          const [wx, wz] = toWorld(frameOf(hull), cx, cz);
          p.x = num(wx, bound);
          p.z = num(wz, bound);
          p.deck = { boat: m.on, x: cx, y: Math.max(0, ly), z: cz, yaw: lyaw };
        }
      }
    ...
    if (m.t === 'boat') {
      ...
      // A crew (Plans/lopen-op-de-boot.md). No page sends these yet: every boat is still a
      // Benchy with room for her pilot alone. Aboard from where the sea last had them.
      else if (m.a === 'board') change = boats.board(m.id, p.id, [p.x, p.z]);
      else if (m.a === 'leave') { change = boats.leave(m.id, p.id); if (change) p.deck = null; }
      else if (m.a === 'letgo') change = boats.letGo(m.id, p.id);
      if (!change) return;
    ...
    // A swing is the sea's to resolve (lib/combat.mjs), and everybody else's to see: the arm
    // coming down, on the hand it was (Plans/andere-spelers-zoals-jij.md). An event rather
    // than a pose bit - a swing is over in less than two pose beats. A drink is only seen.
    if (m.t === 'swing') {
      swing(p);
      if (p.walking) sendAll({ t: 'swung', id: p.id, side: sideOf(m.side) }, p);
      return;
    }
    if (m.t === 'drink') {
      if (p.walking) sendAll({ t: 'drank', id: p.id, side: sideOf(m.side) }, p);
      return;
    }
  ...
  function tick() {
    ...
    // Who is standing on which deck, and where on it, beside the rows rather than in them:
    // a row's slots are fixed (above), and an older page reads the world position in the row
    // and never sees this. A newer one draws the passenger on the hull it is interpolating
    // (web/js/peers.js), which is the only way a passenger stays on a moving boat.
    const d = [];
    for (const p of players.values()) {
      if (p.walking && p.deck) d.push([p.id, p.deck.boat, round(p.deck.x), round(p.deck.y), round(p.deck.z), round(p.deck.yaw)]);
    }
    const snap = JSON.stringify(d.length ? { t: 's', a, d } : { t: 's', a });
    const now = Date.now();
  ...
  function evict(p, at, islandName, boat = null) {
    ...
    for (const b of boats.release(p.id)) sendAll({ t: 'boat', ...b });
    [p.x, p.y, p.z] = at;
    p.room = null; p.at = null; p.f = 0; p.deck = null;
