...
export function createRoster({ maxPlayers = 16, bound = 200, log = () => {} } = {}) {
  ...
  function tick() {
    ...
    for (const p of players.values()) {
      ...
      // The room goes on the end, and only when there is one. Outdoors the row is exactly
      // the six fields it always was, so a client that has never heard of rooms reads it
      // unchanged -- and one that has, reads undefined and calls that outdoors.
      const row = [p.id, round(p.x), round(p.y), round(p.z), round(p.yaw), p.f];
      if (p.room) row.push(p.room);
      a.push(row);
    }
