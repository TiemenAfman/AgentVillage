...
// A session's subagents are not all in one flat folder. A plain Agent call lands in
// `subagents/`, but a workflow puts its fleet in `subagents/workflows/<runId>/`, and a
// hundred agents in one run would otherwise never reach the island at all. So walk the
// tree, and remember which run an apprentice belonged to.
const SUBAGENT_DEPTH = 3;

// The same apprentice can sit in two project folders at once: a session that starts in
// a git worktree is filed under the worktree's own slug *and* under the one of the
// repository it hangs under, and both folders keep a copy of its `agent-<id>.jsonl`.
// Discovering it twice would put up two sheds with one id, so keep one per session and
// agent - the copy written last, which is the one the session kept appending to.
function keepNewest(out, rec) {
  const key = `${rec.sessionId}:${rec.agentId}`;
  const prev = out.get(key);
  if (prev && (prev.mtimeMs > rec.mtimeMs || (prev.mtimeMs === rec.mtimeMs && prev.size >= rec.size))) return;
  out.set(key, rec);
}

function collectSubagents(dir, sessionId, out, run = null, depth = 0) {
  for (const ent of readdirSafe(dir, { withFileTypes: true })) {
    if (ent.isDirectory()) {
      if (depth >= SUBAGENT_DEPTH) continue;
      // `subagents/workflows/wf_1a2b3c` - the run is the folder under `workflows`
      const nextRun = /^wf_[a-z0-9-]+$/i.test(ent.name) ? ent.name : run;
      collectSubagents(path.join(dir, ent.name), sessionId, out, nextRun, depth + 1);
      continue;
    }
    const am = /^agent-(.+)\.jsonl$/i.exec(ent.name);
    if (!am) continue;
    const file = path.join(dir, ent.name);
    const st = statSafe(file);
    if (!st) continue;
    keepNewest(out, {
      sessionId, agentId: am[1], file, size: st.size, mtimeMs: st.mtimeMs, run,
      meta: readJson(path.join(dir, `agent-${am[1]}.meta.json`), null),
    });
  }
}
...
export function discover({ claudeHome = CLAUDE_HOME, desktopDir = DESKTOP_DIR } = {}) {
  const transcripts = [];
  const subagents = new Map();   // `<sessionId>:<agentId>` -> one record, see keepNewest
  const locks = [];
  ...
  for (const slugDir of readdirSafe(projects, { withFileTypes: true })) {
    ...
    for (const ent of readdirSafe(dir, { withFileTypes: true })) {
      if (ent.isFile()) {
        ...
        collectSubagents(path.join(dir, ent.name, 'subagents'), ent.name, subagents);
      }
    }
  }
  ...
  return { transcripts, subagents: [...subagents.values()], locks, desktop, cowork, released };
}
