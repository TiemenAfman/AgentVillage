...
export function createCombat({ fleet, crowds, roster, guards, residents, broadcast = () => {}, now = () => Date.now() }) {
  ...
  // filter that decides who can be hurt), not in the water - no swing there, as the page has
  // none - and not behind a raised shield: a blocking arm is not a swinging one, so a player
  // chooses between the two rather than holding both. A swing that finds nobody costs the
  // player their cooldown all the same (it was a swing) and sends nothing to anybody.
  function swing(p) {
    const t = now();
    if (!p || !afoot(p, pilotsOf(roster))) return false;
    if (p.f & (POSE.SWIMMING | POSE.BLOCKING)) return false;
