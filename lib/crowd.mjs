...
export function createCrowd(island, { known = null } = {}) {
  ...
  return {
    ...
    count: () => spawned,
    // The bell. Rung by createCrowds off the world's clock, never by the crowd itself: a
    // crowd knows its island and nothing about what time it is anywhere.
    setGather: walk.setGather,
    advance(ticks, night) {
...
export function createCrowds({ now = () => Date.now(), maxTicksPerBeat = 8 } = {}) {
  ...
  let owed = 0;
  let ticks = 0;
  // Which gathering is on, from shared/gatherings.mjs, or null. Held here rather than
  // re-derived by whoever asks, so the sea's log and `/health` say the same thing the
  // crowds were told.
  let gathering = null;

  // Whose afternoon it is: this process's own wall clock, which is the same `tz` the
  // welcome hands every client - the sea's clock is the world's, and the page asks
  // shared/gatherings.mjs the same question with the same two numbers. Read at `t` and
  // not once at startup, because the offset changes with the clocks in spring and autumn.
  function bellAt(t) {
    const clock = worldClock(t, -new Date(t).getTimezoneOffset());
    return gatheringAt(clock.day, clock.hour);
  }
  ...
  function join(island) {
    ...
    // Told at once rather than on the next beat: a village that republishes in the middle
    // of lunch has just been walked back to its doorsteps, and it should turn straight
    // round rather than stand there for a tick with the bell already rung.
    crowd.setGather(!!gathering);
  ...
  function tick(nightAmount = 0) {
    ...
    // Ring the bell before anybody moves, so the poses that go out on this beat are of a
    // village that has heard it. Once per beat for all crowds, not once per crowd: it is
    // one question about one clock. This is the call that stayed behind in the browser
    // when the walk came out here - web/js/main.js said "whether they go is the sea's
    // decision" and the sea, for a while, never decided.
    gathering = bellAt(t);
    for (const crowd of crowds.values()) {
      crowd.setGather(!!gathering);
      crowd.advance(n, nightAmount);
    }
    ticks += n;
    return n;
  }
  ...
  return {
    ...
    ticks: () => ticks,
    // The gathering the crowds were last told about, or null. As of the last beat that
    // stepped anybody, which is a beat late at most.
    gathering: () => gathering,
    count: () => crowds.size,
    all: () => [...crowds.values()],
