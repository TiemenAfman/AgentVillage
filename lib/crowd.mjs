...
export function createCrowd(island, { known = null, before = null } = {}) {
  ...
  // And the two who keep a building rather than live in a house: the innkeeper at the
  // tavern, the mayor at the town hall (Plans/kroegbaas-en-burgemeester.md). After the
  ...
  for (const spec of bundle.buildings || []) {
    ...
    walk.spawn(spec.id, spec, [at[0], 0, at[1]], { post: keeper.post, reach, radius: 0.45 }, residentLook(spec).height);
    spawned++;
  }
