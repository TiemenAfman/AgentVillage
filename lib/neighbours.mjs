...
function parseBeacon(buf) {
  ...
  return {
    ...
    dev: text(m.dev, 40) || null,
    // The port this machine is offering a *sea* on, if it is offering one. An island on
    // its own is somewhere to look at; an island hosting a sea is somewhere to join, and
    // that is the thing the server picker is actually hunting for. Carried on the beacon
    // that already exists rather than on a socket of its own: the machinery for finding
    // people on a network was built once and works.
    sea: whole(m.sea, 1, 65535),
    seaName: text(m.seaName, 32) || null,
  };
}

export function createNeighbours({
  // `islandName` is a string or a function handing one back. Hosting renames the island
  // (see islandNameOf in lib/paths.mjs) and /api/sea can start hosting at any moment, so
  // serve.mjs passes a function; a plain string is still fine and is what the tests give.
  port, name, islandName, seed, gridSize, announcing = false,
  // Where this machine is hosting a sea, if it is. A function rather than a number
  // because the sea starts after the beacon does, and a beacon that captured `null` at
  // construction would never mention it.
  sea = () => null,
  settlers = () => 0, dev = () => null, log = () => {}, onChange = () => {},
} = {}) {
  const me = instanceId(port);
  // Named for who it is about: list() below destructures a neighbour's own `island` out of
  // a beacon, and two different islands under one identifier is a trap not worth setting.
  const ourIsland = typeof islandName === 'function' ? islandName : () => islandName;
  ...
  const list = () => [...seen.values()]
    .map(({ id, name: who, island, address, port: p, seed: s, gridSize: g, settlers: n, dev: d, sea: sp, seaName, firstSeen }) => ({
      id, name: who, island, address, port: p, seed: s, gridSize: g, settlers: n, dev: d, firstSeen,
      url: `http://${address}:${p}/`,
      // The sea this machine is hosting, ready to be offered in a server picker.
      sea: sp ? { url: `http://${address}:${sp}/`, name: seaName || `${island}'s sea`, from: 'network' } : null,
    }))
  ...
  function announce(bye = false) {
    ...
    const body = Buffer.from(JSON.stringify({
      ...
      name, island: ourIsland(), port, seed, gridSize, settlers: settlers(),
      dev: dev() || undefined,
      ...(sea() || {}),
    }), 'utf8');
  ...
  function onMessage(buf, rinfo) {
    ...
    if (!had || had.name !== b.name || had.island !== b.island || had.settlers !== b.settlers || had.dev !== b.dev) {
