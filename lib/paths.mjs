...
// What an island founded today is given, on top of the defaults: a small island on a big
// grid, which grows (Plans/eiland-laten-groeien.md). Not the defaults themselves, because a
// default is also what an *old* config.json is filled in with, and an island that stood on
// the default 64 without saying so would be handed a grid of 256 and planned again from
// nothing. Applied only where a config is written for the first time: here in loadConfig
// when there is none, and by scripts/setup.mjs --first-run.
export const FOUNDING = { gridSize: 256, minGridSize: 32 };
