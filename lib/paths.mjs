...
// A linked worktree (`git worktree add`, .claude/worktrees/) has a .git *file* pointing
// at the main repository; the main checkout has the folder.
export const WORKTREE = isFile(path.join(ROOT, '.git'));
export const SHARED_HOME = path.join(os.homedir(), '.promptholm');
...
// which is only the code. One place for a release and a checkout alike, ~/.promptholm, so
// a new release runs on the island a debug build left and the other way round
// (Plans/een-thuis-voor-het-eiland.md). Not AppData: the Claude desktop app is an MSIX
// package, and every AppData write by anything it starts - the session hook, which scans
// and so writes layout.json on every session, and an islander a session restarts - lands
// in the package's own LocalCache, a second layout.json nobody else sees. A linked
// worktree stays a sandbox with its island in itself, or a preview server in one would
// work on the real island and publish to the sea under its name. Decided from the files
// alone, never from how the process was started: the server, the scanner, setup and the
// session hook all have to land on the same answer, and the hook is started by Claude Code
// with nothing of ours in its environment. home() in src-tauri/src/island.rs is the same
// rule and has to stay it. PROMPTHOLM_HOME overrides both, for tests and for somebody who
// wants it elsewhere.
export const HOME = process.env.PROMPTHOLM_HOME || (WORKTREE ? ROOT : settleHome());
...
// ---- moving in -------------------------------------------------------------------
// Everything below runs while this module is still being evaluated, before HOME exists, so
// it may use only what is declared above HOME and function declarations - a `const` further
// down is not there yet.

function isFile(p) {
  try { return fs.statSync(p).isFile(); } catch { return false; }
}

// Where the island in `root` lived before there was one home: a release in AppData, a
// checkout in itself.
export function legacyHome(root) {
  return fs.existsSync(path.join(root, 'release.json')) ? path.join(LOCAL_APPDATA, 'Promptholm') : root;
}

// ~/.promptholm, with the island moved into it if it is not there yet. Moved here, at
// import, rather than by a call somewhere: every process that can write the island imports
// this module, the session hook included, and a list of callers would be one more thing to
// forget. A move that fails leaves this process on the island where it was - never on an
// empty island founded beside a real one that simply had not come over yet.
function settleHome() {
  const home = SHARED_HOME;
  const settled = () => isFile(path.join(home, 'config.json'));
  let from = null;
  try {
    if (settled()) return home;
    from = islandToMove(home);
    if (from) moveIn(from, home);
  } catch (e) {
    try { process.stderr.write(`[promptholm] could not move the island to ${home}: ${e.message}\n`); } catch { /* no stderr either */ }
  }
  return from && !settled() ? from : home;
}

// The island to move in, first of three that has a config.json: the one the session hook
// points at - where the sessions have been going all along, so the real one, and on a
// developer's machine the checkout, since setup --first-run leaves a hook it finds alone -
// then this code's own old home, then a release's, for a fresh clone on a machine that had
// only ever run a release.
function islandToMove(home) {
  const same = (a, b) => path.resolve(a).toLowerCase() === path.resolve(b).toLowerCase();
  for (const c of [hookedHome(), legacyHome(ROOT), path.join(LOCAL_APPDATA, 'Promptholm')]) {
    if (c && !same(c, home) && isFile(path.join(c, 'config.json'))) return path.resolve(c);
  }
  return null;
}

function hookedHome() {
  const hooks = (readJson(path.join(CLAUDE_HOME, 'settings.json'), null) || {}).hooks;
  for (const groups of Object.values(hooks && typeof hooks === 'object' ? hooks : {})) {
    for (const group of Array.isArray(groups) ? groups : []) {
      for (const h of (group && Array.isArray(group.hooks)) ? group.hooks : []) {
        const m = typeof h.command === 'string' && /"([^"]*on-session\.mjs)"|(\S*on-session\.mjs)/.exec(h.command);
        if (m) return legacyHome(path.dirname(path.dirname(path.resolve(m[1] || m[2]))));
      }
    }
  }
  return null;
...
// A copy, not a move: what is left behind is the backup, with a note saying where the island
// went. config.json goes last and by rename, because it is what says the island lives here -
// a copy cut short is simply tried again by the next process. One process copies under
// moving.lock and the rest wait for it: `node --test` starts a dozen at once, and the hook
// and the server can start together.
function moveIn(from, home) {
  fs.mkdirSync(home, { recursive: true });
  const lock = path.join(home, 'moving.lock');
  if (!takeMoveLock(lock)) {
    const until = Date.now() + 120000;
    const pause = new Int32Array(new SharedArrayBuffer(4));
    while (Date.now() < until && fs.existsSync(lock) && !fs.existsSync(path.join(home, 'config.json'))) {
  ...
  try {
    ...
    // Logs stay with the island they describe, locks and half-written files with nobody,
    // guests/ is swept at every start anyway, and print/ is export-stl's, under the checkout.
    const leftBehind = /^(server\.log(\.\d+)?|hook\.log(\.\d+)?|.*\.lock|.*\.tmp|\.gitkeep|MOVED\.txt)$/;
    const data = path.join(from, 'data');
    if (fs.existsSync(data)) {
      fs.cpSync(data, path.join(home, 'data'), {
        recursive: true,
        force: true,
        filter: (src) => {
          const rel = path.relative(data, src).split(path.sep)[0];
          return rel !== 'guests' && rel !== 'print' && !leftBehind.test(path.basename(src));
        },
      });
    }
    if (isFile(path.join(from, '.env'))) fs.copyFileSync(path.join(from, '.env'), path.join(home, '.env'));
    const tmp = path.join(home, `config.json.${process.pid}.tmp`);
    fs.copyFileSync(path.join(from, 'config.json'), tmp);
    fs.renameSync(tmp, path.join(home, 'config.json'));
    try {
      fs.mkdirSync(data, { recursive: true });
      fs.writeFileSync(path.join(data, 'MOVED.txt'),
        `This island moved to ${home} on ${new Date().toISOString()}.\n`
        + `What is here is the copy it was moved from; nothing reads it any more.\n`);
    } catch { /* a note, not a requirement */ }
    process.stderr.write(`[promptholm] the island moved from ${from} to ${home}; the copy there is left as it was\n`);
  } finally {
    try { fs.unlinkSync(lock); } catch { /* gone already */ }
  }
}

function takeMoveLock(file) {
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      const other = readJson(file, null);
      let alive = false;
      try { process.kill(other && other.pid, 0); alive = true; } catch (k) { alive = k.code === 'EPERM'; }
      if (other && alive && Date.now() - (other.at || 0) < 120000) return false;
      try { fs.unlinkSync(file); } catch { /* somebody else cleared it */ }
    }
  }
  return false;
}
...
const DEFAULT_CONFIG = {
  ...
  // The story animals (Plans/dierenverhalen.md). `pace` divides every story duration - the
  // activity window, the day, the gap between two arrivals - so a keeper trying them out can
  // play a week in an afternoon; 1 is real time. Off means the journal is never opened and no
  // animal is published; turning it on again picks the story up where it stopped.
  animals: { enabled: true, pace: 1 },
