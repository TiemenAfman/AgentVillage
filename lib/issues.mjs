...
import { DATA, ROOT, readJson, writeJsonAtomic, loadConfig, DEFAULT_GITHUB_DISPATCH as DEFAULT_DISPATCH } from './paths.mjs';
...
// line is the phrase the issue-oppakken skill triggers on. It moved to lib/paths.mjs (as
// DEFAULT_GITHUB_DISPATCH, imported above under its old name) so that config.json can be
// written out carrying the wording this file actually falls back to.
...
export function githubConfig() {
  ...
  const repo = String(cfg.repo || process.env.PROMPTHOLM_GITHUB_REPO || repoFromRemote() || '').trim();
  return {
