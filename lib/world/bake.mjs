...
export function bakeField(seed, { envelopeM = 1024, radiusM = 208, erosion: strength = WEATHERING, onProgress = null } = {}) {
  ...
  // `mainC` as well as `coast`, for the same reason pass 2 needs both: fresh water belongs to
  // the main island, and deciding that off the union makes the whole bake depend on which
  // skerries happen to fall inside the window.
  const water = carveWater(
    { height, coast, main: mainC, n, envelopeM, metresPerSample: METRES_PER_SAMPLE },
