...
export function bakeField(seed, { envelopeM = 1024, radiusM = 208, onProgress = null } = {}) {
  ...
  for (let j = 0; j < n; j++) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      classes[k] = classifier.classify(worldX(i), z, height[k], coast[k], Math.hypot(gx, gz));
    }
    if (onProgress && j % 64 === 0) onProgress(0.78 + (j / n) * 0.22);
  }
  if (onProgress) onProgress(1);

  return { seed, n, envelopeM, radiusM, metresPerSample: METRES_PER_SAMPLE, height, classes, coast, cappedSamples: capped, shape, relief };
}
