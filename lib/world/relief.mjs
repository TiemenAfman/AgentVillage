...
const SEABED_FLOOR = -32;             // metres: below this the ocean is a plain
...
export function makeRelief(seed, shape) {
   ...
   */
  /**
   * @param d       metres inland, negative at sea. Pass it in when you already have it: the
   *                bake pass computes the coast field once over the whole grid, and letting
   *                this resample it made every height cost five coast evaluations instead of
   *                one - `beachWidth` alone needs four, for the gradient.
   * @param wBeach  beach width at this point, likewise.
   */
  function height(x, z, d = shape.coast(x, z), wBeach = null) {

    if (d <= 0) {
      // Seaward: a shelving foreshore, then the bottom falls away to a floor.
      //
      // The floor matters for more than realism. Without it the seabed followed the warped
      // coast field all the way to the rim of the envelope, so every sample of open ocean was
      // a different number and a chunk of pure sea compressed to 5 kB - as much as a chunk of
      // island. Levelling off makes the deep ocean constant, which is what it looks like and
      // what gzip wants.
      const out = -d;
      if (out < 25 * scale) return -0.06 * out;
      if (out < 90 * scale) return -1.5 - 0.075 * (out - 25 * scale);
      const shelf = -6.4 - 0.12 * (out - 90 * scale);
      return Math.max(shelf, SEABED_FLOOR * scale);
    }

    if (wBeach === null) wBeach = beachWidth(x, z);
