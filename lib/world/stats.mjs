 ...
 * @param {{radiusM?: number, envelopeM?: number, erosion?: number}} opts
 */
export function islandStats(seed, { radiusM = 208, envelopeM = 640, erosion = undefined } = {}) {
  const key = `${seed}:${radiusM}:${envelopeM}:${erosion}`;
  const hit = memo.get(key);
  if (hit) return hit;
  const measured = measureIsland(seed, radiusM, envelopeM, erosion);
  if (memo.size > 32) memo.delete(memo.keys().next().value);
  memo.set(key, measured);
  return measured;
}

function measureIsland(seed, radiusM, envelopeM, erosion) {
  const field = bakeField(seed, { envelopeM, radiusM, erosion });
  const { n, height, classes, shape } = field;
  const half = envelopeM / 2;

  const tally = new Map();
  const slopes = [];
  let land = 0, maxHeight = -Infinity, volumeM3 = 0;
  ...
  return {
    ...
    maxHeight,
    // The volume standing above the waterline. Erosion is a redistribution, so this is what
    // says whether the pass is quietly washing the island away instead of rearranging it.
    volumeM3,
    slope: { p25: at(0.25), p50: at(0.5), p75: at(0.75), p90: at(0.9), p99: at(0.99) },
    classes: classShares,
