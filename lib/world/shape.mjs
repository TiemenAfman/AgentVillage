...
export function makeShape(seed, { radiusM = 208, envelopeM = 1024, islets = ISLETS } = {}) {
  ...
  const skerries = [];
  {
    ...
    const reach = envelopeM / 2;
    const rings = [reach * 0.58, reach * 0.80];
    ...
    for (let k = 0; k < islets; k++) {
      ...
      const dist = ring + rr.range(-28, 28) * scale;
      ...
      for (let i = 0; i < lobeCount; i++) {
        ...
        lobes.push({
          x: cx + Math.cos(a) * step,
          z: cz + Math.sin(a) * step,
          r: (16 + rr.range(0, 26)) * scale,
        });
      }
      skerries.push({ index: k, x: cx, z: cz, lobes });
    }
  }
