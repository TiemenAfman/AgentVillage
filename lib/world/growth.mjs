...
// islet is 0.46 to 1.14 ha and holds four to twenty-two super-cells - a hamlet on each and a
// village on the best of them (measured over seeds 1337, 4242 and 90210 at a 1024 m envelope).
// The mainland comfortably holds the village this island has today, so the settler ladder is
// slack: it is the floor that keeps a single-project island growing at all, not the thing that
// usually fires.
//
 ...
 *
 * The skerries are all decided at t=0 and depend on nothing but the seed and the island's own
 * radius (see `shape.mjs`), so islet 4 is islet 4 whether or not islets 1 to 3 have been
 * published, and whichever window the bake ran in. That index is the
...
// One thing the chunk quantum makes unavoidable: a channel is 21 to 36 m wide and a chunk
// covers 64, so a chunk that holds the mainland's coast sometimes holds the near tip of a
// skerry as well. That chunk comes up with the mainland, so a sandbar of an islet nobody has
// earned yet breaks the surface. The alternative - holding the chunk back until every landmass
// in it is published - punches a hole in the mainland's own coast, which is worse and visible
// from the town. The tip carries no landmass entry in the manifest, so nothing offers it to a
// district as an island.
//
// Measured on seed 1337, this got better rather than worse when the archipelago grew from six
// rocks to sixteen: 7 shared chunks instead of 18. The old fan measured its gap along one
// bearing only and let a rock drift to 1.4 m of the shore off that bearing; the walk in
// `shape.mjs` clears the whole coastline.
