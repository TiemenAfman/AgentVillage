...
export function findLandmasses(field) {
  ...
  const masses = [];
  const stack = [];
  // Its own counter, and not `masses.length`: a rock too small to be named is labelled and
  // then dropped, so numbering by the list length hands the next real landmass the label the
  // rock already wears. Two landmasses under one number is not a cosmetic problem - it is how
  // a speck of an unpublished islet ends up wearing the mainland's group, or how a skerry
  // reads as 1.4 m from another one that is nowhere near it.
  let next = 0;
