...
// not have loadConfig found an island in the checkout they run in. `onChange` is told when what
// the sea should hear has changed; `{ again: true }` means tell it even if the bytes are the
// same, because the sea's answer to them is what was lost.
export function createAnimalLife({ dir, pace = 1, log = () => {}, now = () => Date.now(), onChange = () => {}, onNews = () => {}, check = null } = {}) {
  ...
  function open() {
    if (store && !store.healthy()) recover();
  ...
  // A write failed - a full disk, a file somebody had open - and the store refuses everything
  // after it rather than append behind a line it may have half-written. Without this the
  // animals stood still until the islander was restarted, and the one memory the failed line
  // held was never written at all: the sea had answered, and nobody asked it again. So the
  // store is closed and opened again, which replays the journal as far as it is whole (a torn
  // tail goes aside, as on any open), and the sea is asked to hear everything once more -
  // forced, because nothing it is told has changed - so it answers every errand it finished
  // out of what it remembers (lib/animal-crowd.mjs `finished`). If the disk is still refusing,
  // the reopen fails like a first open does and the animals stay away with the reason kept.
  function recover() {
    const was = store;
    store = null;
    try { was.close(); } catch { /* the lock goes with it either way */ }
    log('the animals\' journal could not be written; opening it again');
    try {
      store = openAnimalStore(dir, { rules, recoverStale: true });
      sinceCheckpoint = 0;
      onChange({ again: true });
  ...
  function arrive(species, residents, p) {
    ...
    for (const b of order) {
      ...
      // The island's seed and its founding: a great many islands share seed 1337
      // (config.example.json), and they should not all be greeted by the same restless hen.
      const seed = `${village.island.seed}:${village.island.foundedAt || ''}`;
      const e = run({ kind: 'arrive', species, home, near: b.id, lookout, baseline, seed });
