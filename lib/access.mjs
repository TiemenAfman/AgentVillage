...
export function createAccess({ port, config, boardOrigin = null }) {
  ...
  function originOk(req, p) {
    ...
    // own pages fetches its module scripts from there. Narrow on purpose: only that one
    // origin, and only the static files isSharedWithBoards lists. A POST to /api/ from
    // the same place is still refused, which is the whole point of this check.
    if (boardOrigin && origin === boardOrigin && p && isSharedWithBoards(p)) return true;
    try {
