...
const PUBLIC_EXACT = new Set([
  ...
  // Scenery, like /api/props: a visitor who can see the board standing there should be
  // able to read what is on it. It fetches nothing but the sites already on the island -
  // see lib/billboard.mjs - so opening it opens no more than the island already shows.
  '/billboard',
]);
const PUBLIC_PREFIX = ['/js/', '/css/', '/vendor/', '/icons/', '/shared/', '/fonts/'];

// The same static files, and *only* these, may also be read from the billboard port -
// which is a different origin, so a browser will not fetch them across without being
// told. That matters for exactly one thing: a board carrying a page of the island's own,
// whose <script type="module"> is refused cross-origin without it. Classic scripts, the
// kind most sites use, never needed this.
//
// It is a list rather than a blanket rule because the API must never be on it. These are
// files any visitor may already GET; letting a page on the board port read them as well
// gives away nothing that was not public, while `Access-Control-Allow-Origin` on /api/
// would hand a framed stranger the island.
export function isSharedWithBoards(p) {
...
export function createAccess({ port, config, boardPort = null }) {
  ...
  function originOk(req, p) {
    const origin = req.headers.origin;
    if (!origin) return true;                     // curl, the scheduled task, a same-origin GET
    // The billboard port is an origin of ours, and a board carrying one of the island's
    // own pages fetches its module scripts from there. Narrow on purpose: the same host
    // the request already came to, only on the board port, and only for the static files
    // isSharedWithBoards lists. A POST to /api/ from that same place is still refused,
    // which is the whole point of this check.
    //
    // Matched on host rather than on a fixed origin because the island answers to more
    // than one name - localhost here, an address on the network there - and its boards
    // have to be reachable by whichever name was used to get here.
    if (boardPort && p && isSharedWithBoards(p)) {
      try {
        const o = new URL(origin);
        const here = hostName(req.headers.host);
        if (o.protocol === 'http:' && o.port === String(boardPort) && hostName(o.host) === here) return true;
      } catch { /* falls through to the ordinary check */ }
    }
    try {
  ...
  function classify(req, url) {
    ...
    if (!originOk(req, url && url.pathname ? decodeURIComponent(url.pathname) : '')) return { role: 'refused', why: `origin ${req.headers.origin}` };
