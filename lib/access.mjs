...
//
// All three, always. There was briefly a fourth paragraph here - the in-place join spent
// the Origin, narrowly and behind a flag, so a page on this machine could talk to a
// neighbour's island. The sea made that unnecessary: the world is somewhere else now and
// this server answers nothing but its own machine, so the allowance and the write route it
// was built around are both gone. This file is back to what it was, which is the shortest
...
//
// There is no write route on this list, and there is not meant to be. There was one -
// POST /api/island, which took delivery of somebody else's island and parked it under
// data/guests/ - and it was the only thing this file has ever let a visitor write. The sea
// took that job: an island is published to a world that holds it in memory and never
// writes it down, so nothing arrives here from outside any more and an inviteCode is back
// to buying a look.
...
export function createAccess({ port, config }) {
  ...
  // An Origin that is absent or matches the Host, and nothing else. A request carrying a
  // foreign one is a page on another site talking to this machine, whatever it says it is.
