...
nothing else. `npm run setup` puts one in (`hooks/statusline.mjs`) that writes the window
down in `data/usage.json` and shows `⛏ 65/100 gold` at the bottom of the terminal. If you
already had a status line of your own it is kept: the island's goes in front of it in a
pipe (`… statusline.mjs --pass | your-command`) and hands it exactly the input it always
got, and `node scripts/setup.mjs --uninstall` gives it back as it was. The reading is only
there for a Pro or Max subscription, and only once a session has had its first answer.
