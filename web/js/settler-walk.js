// Where the settlers go. Half of what used to be web/js/settlers.js, and the half with no
// opinion about how any of it looks.
//
// The split is along one line: this file decides *where a body is*, and settler-figures.js
// decides what is drawn there. Nothing in here imports three.js, touches a mesh or reads a
// colour, which is not tidiness for its own sake - it is what lets the same code run in
// Node, where there is no renderer and no document, so the island can keep living while
// nobody has a tab open. web/js/settlers.js stitches the two halves back together and is
// still the only thing the rest of the island imports.
//
// The happiest thing found while pulling them apart: none of the cosmetic sine waves feed
// back into position. The bob, the gait, the arm swing and the idle sway are all derived
// from the renderer's own clock and never from anything this file computes - so they never
// have to be deterministic, never have to be on a wire, and did not have to come along.
// What crosses the seam instead is one word per figure: `f.anim`, saying which of the four
// animations the body is in this frame. See `step`.
//
// One figure object is still shared between the two halves rather than split in two. That
// is deliberate for now: main.js, facetoface.js and boating.js all reach into `figures` and
// read `f.pos`, `f.look` and `f.spec` directly, and splitting the object as well as the
// code would have made this a rewrite of five files instead of a move. Each half's fields
// are grouped and labelled below, which is what will make that later split mechanical.
...
export function createWalk(terrain) {
  ...
  return {
    ...
    setRoads, setDecks, setGather, walkIn, step,
