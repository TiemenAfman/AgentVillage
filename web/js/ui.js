...
export function createUI(handlers) {
  ...
  let indoors = false;
  // Whether each mouse button drinks rather than fights: a beer in the hand that answers to
  // it (Plans/bier-en-dronken.md). main.js asks the walk every frame, so only a change - a
  // pint picked up in the inventory, a room entered - redraws the row.
  let lmbDrinks = false, rmbDrinks = false;
  function setMouse(lmb, rmb) {
    if (!!lmb === lmbDrinks && !!rmb === rmbDrinks) return;
    lmbDrinks = !!lmb; rmbDrinks = !!rmb;
    renderWalkKeys();
  }
  ...
  function renderWalkKeys() {
    ...
    const drinks = (lmbDrinks ? '<span><kbd>LMB</kbd> drink</span>' : '') + (rmbDrinks ? '<span><kbd>RMB</kbd> drink</span>' : '');
    if (indoors) {
    ...
    el('walk-keys').innerHTML = padConnected
      ...
      : `<span><kbd>W</kbd><kbd>A</kbd><kbd>S</kbd><kbd>D</kbd> walk</span><span>mouse to look, <kbd>Esc</kbd> frees it, click takes it back</span>`
        + (lmbDrinks ? '<span><kbd>LMB</kbd> drink</span>' : '<span><kbd>LMB</kbd> attack</span>')
        + (rmbDrinks ? '<span><kbd>RMB</kbd> drink</span>' : '<span><kbd>RMB</kbd> hold to block</span>')
