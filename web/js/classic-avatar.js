...
export function createClassicAvatar(spec, material) {
  ...
  // Drinking (Plans/bier-en-dronken.md). A beer answers to whichever button its hand already
  // had - the swinging hand's left, the blocking hand's right, the same attackSide/blockSide
  // as above, in which a beer is no weapon - and lifts the glass instead: up to the face and
  // a little inward, a swallow with the glass tipped towards the mouth, and back down. Timed
  ...
  let gulped = 0;
  const sideFor = (role) => (role === 'block' ? blockSide() : attackSide());
  // Whether `role`'s button ('attack' or 'block') is a beer's - asked before walk.js fights,
  // so a click on it never becomes a swing or a raised arm the sea would believe.
  const holdsBeer = (role) => holding[sideFor(role)] === 'beer';
  function drink(role) {
    const side = sideFor(role);
  ...
  return { object, update, set, dispose, handAttach, attack, holdsBeer, drink, swallowed };
}
