...
export function createWalk(terrain, village = null) {
  ...
  function spawn(id, spec, worldPos, opts = {}, height = 1) {
    ...
    const f = {
      ...
      attend: null,
      // The gold errand (see `startGold`). `works` is whether their session is running -
      // what they go back to doing when they get home with a bar - and `goldIn` the seconds
      // of work left before the next trip, null until the first tick of work sets it.
      // `carry` is 'gold' while a bar is in their hands, which the sea sends on to every
      // page as its own message (encodeCarry in shared/settlerwire.mjs). `goldRng` is made
      // on first use, off the id: a stream of its own so that nothing here draws from the
      // middle of `rng`.
      works: opts.mode === 'hammer', goldIn: null, goldRng: null, carry: null,
  ...
  function setMode(id, mode) {
    const f = figures.get(id);
    if (!f) return;
    if (mode === 'hammer' || mode === 'idle') f.works = mode === 'hammer';
  ...
  function adopt(id, old) {
    ...
    if (!out) {
      ...
      if (f.works && old.works) f.goldIn = old.goldIn;
      return f.works && old.works;
    }
    ...
    for (const f of figures.values()) {
      ...
      if (f.aboard) {
        ...
        if (f.pause <= 0) {
          const t = f.then;
          f.then = null;
          if (t && t.kind === 'gold-home') { carryHome(f, t); f.anim = 'walk'; }
          else { f.mode = f.works ? 'hammer' : 'idle'; f.carry = null; goldTrips--; }
        }
      } else if (f.chartered) {
