...
and with `--pass` (setup puts it *in front of* a status line the user already had,
`lib/statusline.mjs`) stdin goes back out untouched before anything that could fail. The
...
stream, `MAX_GOLD` out at once), and who carries one travels as `{t:'fc', i, c}` - the whole
set, when it changes, like `fh` - not as a fifth `ANIMS` word an older page would read as
'still'. `createCrowd(island, { known, before })` hands each settler's gold errand over from
