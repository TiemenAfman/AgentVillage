...
| Wat de pagina ziet | **Wie een staaf draagt loopt als het woord `'carry'`**, achteraan in `ANIMS` en in `MOVING` - precies zoals een houthakker met een bos takken `'haul'` loopt (Plans/inwoners-aan-het-werk.md). De pagina tekent dan een goudstaaf dwars over beide vuisten (`settler-figures.js`). | Op `Develop_Tiemn` is dit al het patroon voor lopen-met-iets-in-de-hand, dus geen tweede mechanisme ernaast. Een oude pagina leest het onbekende getal als `'still'`, wat bij `'haul'` al zo was. (Op `main` bestaat `'haul'` niet en ging het als een eigen `fc`-bericht naast `fh`; die versie staat op `claude/charming-clarke-mf9egn`.) |
