...
```
hammer ──(goldIn op)──> lopen naar de kuil ──> laden (1,6–3 s) ──> met een staaf naar huis ──> hammer
```
...
| Wat de pagina ziet | Onderweg gewoon `walk` (de draad is ongewijzigd). **Wie een staaf draagt** gaat als eigen bericht: `{t:'fc', i, c: [idx, …]}`, de hele set, alleen als hij verandert, plus één keer per eiland bij het joinen. De pagina tekent dan een goudstaaf in beide handen (`settler-figures.js`). | Hetzelfde patroon als `fh` (wie vastgehouden wordt): een nieuw `ANIMS`-woord zou een oude pagina als `still` lezen en glijdende settlers opleveren, en dan moet `SEA_V` omhoog. Een onbekend bericht laat `net.js` gewoon vallen. |
...
| Versie | **`SEA_V` blijft 3**, geen layout-poort. Een oude zee stuurt geen `fc` en laat niemand goud halen; een nieuwe zee met een oude pagina laat ze wel lopen, alleen zonder staaf in de hand. | Alleen ophogen als een oude peer iets verkeerd zou lezen. Wel een nieuwe functie, dus de volgende minor (0.5.0), niet een patch — en een zee op afstand moet opnieuw uitgerold worden voordat het goud daar gehaald wordt. |
