...
| Welke muisknop? | De knop die die hand al had: de hand die zou slaan (`attackSide`) drinkt op links, de hand die zou blokkeren (`blockSide`) drinkt op rechts. Bier telt niet als wapen, dus een zwaard of schild in de andere hand houdt zijn eigen knop. | Dat is letterlijk "de bijbehorende muisknop", en er hoeft niets aan de verdeling van de handen te veranderen. |
| Blokkeert drinken? | **Nee.** De rechterknop op een bierhand zet `state.blocking` nooit. | Anders zou de zee een drinker als blokkerend zien en minder schade laten doen. |
...
- `web/js/classic-avatar.js` — `beerGeometry()`, `holdsBeer(role)`, `drink(role)`, `swallowed()`.
- `web/js/walk.js` — knoppen, `canDrink`, zwalken.
